 ...
 */
public class Graph6Sparse6Importer<V, E> extends AbstractBaseImporter<V, E>
  ...
  public void importGraph(Graph<V, E> g, Reader input) throws ImportException {
    ...
    if (g6.isEmpty()) throw new ImportException("Failed to read graph");
   ...
   *
   * @param g the graph
   * @param g6 String representation of a graph either in graph6 or sparse6 format. WARNING: a g6/s6
   *     string may contain backslashes '\'. Escaping is required when invoking this method
   *     directly. E.g.
   *     <pre><code>importgraph(g,":?@MnDA\oi")</code></pre>
  ...
  public void importGraph(Graph<V, E> g, String g6) throws ImportException {
    ...
    if (g6.startsWith(":")) {
      ...
      this.format = Format.SPARSE6;
    } else if (g6.startsWith(">>graph6<<")) g6 = g6.substring(10, g6.length());
    ...
    if (format == Format.GRAPH6) this.readGraph6(g, map);
    else this.readSparse6(g, map);
  }
    ...
    for (int i = 0; i < vertexIndexMap.size(); i++) {
      for (int j = 0; j < i; j++) {
        ...
        if (bit == 1) {
          ...
          if (g.getType().isWeighted()) g.setEdgeWeight(e, defaultWeight);
        }
      }
    }
  }
