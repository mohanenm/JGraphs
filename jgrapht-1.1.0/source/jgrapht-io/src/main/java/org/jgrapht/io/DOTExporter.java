 ...
 */
public class DOTExporter<V, E> extends AbstractBaseExporter<V, E> implements GraphExporter<V, E> {
   ...
   * @param vertexIDProvider for generating vertex IDs. Must not be null.
   * @param vertexLabelProvider for generating vertex labels. If null, vertex labels will not be
   *     written to the file (unless an attribute provider is supplied which also supplies labels).
   * @param edgeLabelProvider for generating edge labels. If null, edge labels will not be written
   *     to the file.
   * @param vertexAttributeProvider for generating vertex attributes. If null, vertex attributes
   *     will not be written to the file.
   * @param edgeAttributeProvider for generating edge attributes. If null, edge attributes will not
   ...
   * @param edgeAttributeProvider for generating edge attributes. If null, edge attributes will not
   *     be written to the file.
   * @param graphIDProvider for generating the graph ID. If null the graph is named {@link
   *     #DEFAULT_GRAPH_ID}.
   */
  public DOTExporter(
      ComponentNameProvider<V> vertexIDProvider,
  ...
  public void exportGraph(Graph<V, E> g, Writer writer) {
    ...
    String connector;
    String header =
        (g instanceof AbstractBaseGraph && !((AbstractBaseGraph<V, E>) g).isAllowingMultipleEdges())
