 ...
 *
 * <p>For a description of the format see <a href="http://en.wikipedia.org/wiki/DOT_language">
 * http://en.wikipedia.org/wiki/DOT_language</a> and <a
 * href="http://www.graphviz.org/doc/info/lang.html">http://www.graphviz.org/doc/info/lang.html</a>
 *
...
public class DOTImporter<V, E> extends AbstractBaseImporter<V, E> implements GraphImporter<V, E> {
  ...
  private static final CharSequenceTranslator UNESCAPE_ID =
  ...
  public DOTImporter(
    ...
    super(
        vertexProvider,
        edgeProvider,
        (vertexUpdater != null) ? vertexUpdater : (c, a) -> {},
  ...
  public void importGraph(Graph<V, E> g, Reader in) throws ImportException {
    try {
      /**
       * Create lexer with unbuffered input stream and use a token factory which copies characters
       * from the input stream into the text of the tokens.
       */
  ...
  private class CreateGraphDOTListener extends DOTBaseListener {
        ...
        if (cur.attrs != null) {
          ...
          for (V sourceVertex : prev.getVertices()) {
            for (V targetVertex : cur.getVertices()) {
              ...
              Map<String, Attribute> edgeAttrs = new HashMap<>(subgraphScopes.element().edgeAttrs);
              ...
              try {
                ...
                E e = edgeProvider.buildEdge(sourceVertex, targetVertex, edgeLabel, edgeAttrs);
    ...
    public void exitIdentifierPairStatement(DOTParser.IdentifierPairStatementContext ctx) {
      ...
      if (subgraphScopes.size() == 1) {
        try {
          graphUpdater.update(
              graph, Collections.singletonMap(key, DefaultAttribute.createAttribute(value)));
        } catch (Exception e) {
          throw new IllegalArgumentException("Graph update failed: " + e.getMessage(), e);
        }
      }
