 ...
 *
 * <p>In particular, the length of a Graph6 string representation of a graph depends only on the
 * number of vertices. However, this also means that graphs with few edges take as much space as
 * graphs with many edges. On the other hand, Sparse6 is a variable length format which can use
...
public class Graph6Sparse6Exporter<V, E> implements GraphExporter<V, E> {
  ...
  public void exportGraph(Graph<V, E> g, Writer writer) throws ExportException {
    ...
    try {
      if (format == Format.SPARSE6) writeSparse6(g, vertices);
      else writeGraph6(g, vertices);
    } catch (IOException e) {
      e.printStackTrace();
    }
  ...
  private void writeSparse6(Graph<V, E> g, List<V> vertices) throws IOException {
    ...
    if (bitIndex != 0) {
      ...
      for (int i = 0; i < padding; i++) writeBit(true);
  ...
  private void writeGraph6(Graph<V, E> g, List<V> vertices) throws IOException {
    ...
    for (int i = 0; i < vertices.size(); i++)
      for (int j = 0; j < i; j++) writeBit(g.containsEdge(vertices.get(i), vertices.get(j)));
    writeByte(); // Finish writing the last byte
  }

  private void writeNumberOfVertices(int n) throws IOException {
    assert n >= 0;
    if (n <= 62) byteArrayOutputStream.write(n + 63);
    else if (n <= 258047) {
  ...
  private void writeIntInKBits(int number, int k) {
    for (int i = k - 1; i >= 0; i--) writeBit((number & (1 << i)) != 0);
  }

  private void writeBit(boolean bit) {
    if (bitIndex == 6) writeByte();
    if (bit) currentByte |= 1 << (5 - bitIndex);
    bitIndex++;
  }
