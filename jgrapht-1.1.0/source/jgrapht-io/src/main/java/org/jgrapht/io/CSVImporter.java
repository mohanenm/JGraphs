 ...
 * #setParameter(org.jgrapht.io.CSVFormat.Parameter, boolean) setParameter} method. See {@link
...
public class CSVImporter<V, E> extends AbstractBaseImporter<V, E> implements GraphImporter<V, E> {
  ...
  private class MatrixCSVListener extends RowCSVListener {
    ...
    public MatrixCSVListener(Graph<V, E> graph) {
      ...
      this.assumeEdgeWeights = parameters.contains(CSVFormat.Parameter.MATRIX_FORMAT_EDGE_WEIGHTS);
        ...
        try {
          ...
          if (entryAsInteger == 0) {
            ...
            if (assumeEdgeWeights) {
              createEdge(
                  currentVertexName, columnIndex.get(target), Double.valueOf(entryAsInteger));
            } else {
