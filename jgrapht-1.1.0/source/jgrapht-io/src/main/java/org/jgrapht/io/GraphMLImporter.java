 ...
 *
 * <p>For a description of the format see <a href="http://en.wikipedia.org/wiki/GraphML">
 * http://en.wikipedia.org/wiki/ GraphML</a> or the <a
 * href="http://graphml.graphdrawing.org/primer/graphml-primer.html">GraphML Primer</a>.
 *
 * <p>Below is small example of a graph in GraphML format.
 *
 * <pre>{@code
 ...
 * }</pre>
 *
 * <p>The importer reads the input into a graph which is provided by the user. In case the graph is
 ...
 *
 * <p>GraphML-Attributes Values are read as string key-value pairs and passed on to the {@link
 * VertexProvider} and {@link EdgeProvider} respectively.
 *
 * <p>The provided graph object, where the imported graph will be stored, must be able to support
 * the features of the graph that is read. For example if the GraphML file contains self-loops then
 * the graph provided must also support self-loops. The same for multiple edges. Moreover, the
 * parser completely ignores the attribute "edgedefault" which denotes whether an edge is directed
 * or not. Whether edges are directed or not depends on the underlying implementation of the user
 * provided graph object.
 *
 * <p>The importer by default validates the input using the 1.0 <a
 * href="http://graphml.graphdrawing.org/xmlns/1.0/graphml.xsd">GraphML Schema</a>. The user can
 ...
 * @since July 2016
 */
public class GraphMLImporter<V, E> extends AbstractBaseImporter<V, E>
   ...
   * Import a graph.
   *
   * <p>The provided graph must be able to support the features of the graph that is read. For
   ...
   *
   * <p>If the provided graph is a weighted graph, the importer also reads edge weights.
   *
   * <p>GraphML-Attributes Values are read as string key-value pairs and passed on to the {@link
   * VertexProvider} and {@link EdgeProvider} respectively.
   *
      ...
      for (Entry<String, GraphElement> en : nodes.entrySet()) {
        ...
        for (Key validKey : nodeValidKeys.values()) {
          ...
          if (collectedAttributes.containsKey(validId)) {
            ...
            finalAttributes.put(
                validKey.attributeName, new DefaultAttribute<>(validKey.defaultValue, validType));
          }
        }
      ...
      for (GraphElement p : edges) {
        ...
        if (handleSpecialEdgeWeights) {
          if (finalAttributes.containsKey(edgeWeightAttributeName)) {
            try {
              graph.setEdgeWeight(
                  e, Double.parseDouble(finalAttributes.get(edgeWeightAttributeName).getValue()));
      ...
      switch (localName) {
        ...
        case EDGE:
          currentGraphElement.push(
              new GraphElement(
                  findAttribute(EDGE_SOURCE, attributes), findAttribute(EDGE_TARGET, attributes)));
          break;
        case KEY:
