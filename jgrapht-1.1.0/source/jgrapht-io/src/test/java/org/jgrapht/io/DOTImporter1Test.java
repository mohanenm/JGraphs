...
public class DOTImporter1Test {
  ...
  @Test
  public void testImportID() throws ImportException {
    String id = "MyGraph";
  ...
  @Test
  public void testImportWrongID() throws ImportException {
  ...
  @Test
  public void testInvalidHeader() throws ImportException {
    // testing all cases of missing keywords or wrong order
    for (String invalidInput :
        new String[] {
          " {}",
          "strict {}",
          "id {}",
  ...
  @Test
  public void testImportOnlyGraphKeyword() throws ImportException {
    String input = "graph {\n}\n";
    GraphWithID result = new GraphWithID();
    buildGraphIDImporter().importGraph(result, new StringReader(input));
    assertNull(result.id);
  ...
  @Test
  public void testImportNoID() throws ImportException {
  ...
  public void testDirectedNoLabels() throws ImportException {
    ...
    DirectedMultigraph<String, DefaultEdge> expected = new DirectedMultigraph<>(DefaultEdge.class);
    ...
    DirectedMultigraph<String, DefaultEdge> result = new DirectedMultigraph<>(DefaultEdge.class);
    importer.importGraph(result, new StringReader(input));
  ...
  public void testDashLabelVertex() throws ImportException {
    ...
    DOTImporter<String, DefaultEdge> importer = new DOTImporter<String, DefaultEdge>(vp, ep, cu);
  ...
  public void testIncompatibleDirectedGraph() {
    ...
    testGarbageGraph(input, "Failed to import DOT graph: Provided graph is not directed", result);
  }
  ...
  public void testIncompatibleGraph() {
    ...
    testGarbageGraph(input, "Failed to import DOT graph: Provided graph is not undirected", result);
  }

  @Test
  public void testAttributesWithNoValues() throws ImportException {
    String input =
        "graph G {\n"
            + "  1 [ label = \"bob\" \"foo\" ];\n"
            + "  2 [ label = \"fred\" ];\n"
  ...
  public void testLabelsWithEscapedSemicolons() throws ImportException {
    ...
    assertEquals("wrong size of vertexSet", 1, result.vertexSet().size());
    assertEquals("wrong size of edgeSet", 0, result.edgeSet().size());
    assertEquals("wrong parsing", "node0", ((TestVertex) result.vertexSet().toArray()[0]).getId());
    assertEquals(
        "wrong parsing",
  ...
  private static class GraphWithID extends AbstractBaseGraph<String, DefaultEdge> {
