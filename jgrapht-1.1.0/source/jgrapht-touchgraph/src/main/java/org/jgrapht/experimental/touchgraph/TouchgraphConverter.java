...
public class TouchgraphConverter<V, E> {
   ...
   * object is a GraphEltSet but Touchgraph does not provide the visibility to use it easily and one
   * can use a JGraphT graph. While JGraphT nodes can be any type of objects, TouchGraph uses a set
   * of com.touchgraph.graphlayout.Node and com.touchgraph.graphlayout.Edge only. Moreover,
   * TouchGraph edges are always directed. Having said that, if you want a nice way to visualize and
   * explore a graph, especially large complex graphs, TouchGraph is very nice
   *
   ...
   *     to itself? Self-referential loops do not show up in the TG visualization but you may want
   *     to subclass TG's Node class to show them
   * @return first node of the TouchGraph graph
   * @throws TGException in case of an error
   */
  public Node convertToTouchGraph(Graph<V, E> graph, TGPanel tgPanel, boolean selfReferencesAllowed)
      throws TGException {
