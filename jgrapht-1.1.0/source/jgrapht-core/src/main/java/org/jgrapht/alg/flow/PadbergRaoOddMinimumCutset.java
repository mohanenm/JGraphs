 ...
 *
 * <p>The algorithm has been published in: Padberg, M. Rao, M. Odd Minimum Cut-Sets and b-Matchings.
 ...
 *
 * <p>The runtime complexity of this algorithm is dominated by the runtime complexity of the
 * algorithm used to compute A Gomory-Hu tree on graph G. Consequently, the runtime complexity of
 * this class is O(V^4).
 *
 * <p>This class does not support changes to the underlying graph. The behavior of this class is
...
public class PadbergRaoOddMinimumCutset<V, E> {
  ...
  /**
   * Calculates the minimum odd cut. The implementation follows Algorithm 1 in the paper Odd minimum
   * cut sets and b-matchings revisited by Adam Letchford, Gerhard Reinelt and Dirk Theis. The
   * original algorithm runs on a compressed Gomory-Hu tree: a cut-tree with the odd vertices as
   * terminal vertices. This tree has |T|-1 edges as opposed to |V|-1 for a Gomory-Hu tree defined
   * on the input graph G. This compression step can however be skipped. If you want to run the
   * original algorithm in the paper, set the parameter <code>useTreeCompression</code> to true.
   * Alternatively, experiment which setting of this parameter produces the fastest results. Both
   * settings are guaranteed to find the optimal cut. Experiments on random graphs showed that
   * setting <code>useTreeCompression</code> to false was on average a bit faster.
   *
   * @param oddVertices Set of vertices which are labeled 'odd'. Note that the number of vertices in
  ...
  public double calculateMinCut(Set<V> oddVertices, boolean useTreeCompression) {
    ...
    if (useTreeCompression) return calculateMinCutWithTreeCompression();
    else return calculateMinCutWithoutTreeCompression();
  }

  /**
   * Modified implementation of the algorithm proposed in Odd Minimum Cut-sets and b-matchings by
   * Padberg and Rao. The optimal cut is directly computed on the Gomory-Hu tree computed for graph
   * G. This approach iterates efficiently over all possible cuts of the graph (there are |V| such
   * cuts).
   *
   * @return weight of the minimum odd cut.
   */
  private double calculateMinCutWithoutTreeCompression() {
    ...
    for (DefaultWeightedEdge edge : edges) {
      ...
      if (edgeWeight >= minimumCutWeight) continue;
  ...
  /**
   * Implementation of the algorithm proposed in Odd Minimum Cut-sets and b-matchings by Padberg and
   * Rao. The algorithm evaluates at most |T| cuts in the Gomory-Hu tree.
   *
   * @return weight of the minimum odd cut.
   */
  ...
  private void splitCluster(Set<V> cluster, Queue<Set<V>> queue) {
    ...
    if (split1.size() > 1) queue.add(split1);
    if (split2.size() > 1) queue.add(split2);
  }
  ...
  public static <V> boolean isOddVertexSet(Set<V> vertices, Set<V> oddVertices) {
    ...
    else return oddVertices.stream().filter(vertices::contains).count() % 2 == 1;
  }

  /**
   * Returns partition W of the cut obtained after the last invocation of {@link
  ...
  /**
   * Returns partition V-W of the cut obtained after the last invocation of {@link
   * #calculateMinCut(Set, boolean)}
   *
  ...
  /**
   * Returns the set of edges which run from the source partition to the sink partition, in the s-t
   * cut obtained after the last invocation of {@link #calculateMinCut(Set, boolean)}
   *
  ...
  public Set<E> getCutEdges() {
    Predicate<E> predicate =
        e ->
            sourcePartitionMinimumCut.contains(network.getEdgeSource(e))
