...
/**
 * A GraphPath represents a <a href="http://mathworld.wolfram.com/Path.html">path</a> in a {@link
 * Graph}. Unlike some definitions, the path is not required to be a <a
...
public interface GraphPath<V, E> {
   ...
   *
   * <p>Whether or not the returned edge list is modifiable depends on the path implementation.
   *
  ...
  /**
   * Returns the weight assigned to the path. Typically, this will be the sum of the weights of the
   * edge list entries (as defined by the containing graph), but some path implementations may use
   * other definitions.
   *
