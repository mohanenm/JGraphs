 ...
 *
 * <p>The pivoting follows the rule from the paper
 *
 * <ul>
 ...
 * </ul>
 *
 * <p>where the authors show that using that rule guarantees that the Bron-Kerbosch algorithm has
 ...
 *
 * <p>The algorithm first computes all maximal cliques and then returns the result to the user. A
...
public class PivotBronKerboschCliqueFinder<V, E> extends BaseBronKerboschCliqueFinder<V, E> {
  ...
  protected void findCliques(Set<V> P, Set<V> R, Set<V> X, final long nanosTimeLimit) {
    ...
    for (V v : candidates) {
      ...
      Set<V> newP = P.stream().filter(x -> vNeighbors.contains(x)).collect(Collectors.toSet());
      Set<V> newX = X.stream().filter(x -> vNeighbors.contains(x)).collect(Collectors.toSet());
