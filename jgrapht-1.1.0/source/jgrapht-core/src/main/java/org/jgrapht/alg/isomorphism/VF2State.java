...
abstract class VF2State<V, E> {
  ...
  public void addPair() {
    showLog(
        "addPair", "(" + g1.getVertex(addVertex1) + ", " + g2.getVertex(addVertex2) + ") added");
   ...
   *     isomorphic to the already matched vertices of graph2 and the second one vertex of nextPair.
   */
  ...
  public void backtrack() {
    ...
    showLog(
        "backtrack",
        "remove ("
            + g1.getVertex(addedVertex1)
            + ", "
            + g2.getVertex(addedVertex2)
            + ") from the matching");
