 ...
 * than this property, a subgraph is a graph with any respect and fully complies with the <code>
 * Graph</code> interface.
 *
 * <p>If the base graph is a {@link org.jgrapht.ListenableGraph}, the subgraph listens on the base
 ...
 *
 * <p>If the base graph is <i>not</i> a ListenableGraph, then the subgraph property cannot be
 * guaranteed. If edges or vertices are removed from the base graph, they are <i>not</i> removed
 * from the subgraph.
 *
 * <p>Modifications to Subgraph are allowed as long as the subgraph property is maintained. Addition
 * of vertices or edges are allowed as long as they also exist in the base graph. Removal of
 * vertices or edges is always allowed. The base graph is <i>never</i> affected by any modification
 * made to the subgraph.
 *
 * <p>A subgraph may provide a "live-window" on a base graph, so that changes made to its vertices
 * or edges are immediately reflected in the base graph, and vice versa. For that to happen,
 * vertices and edges added to the subgraph must be <i>identical</i> (that is, reference-equal and
 * not only value-equal) to their respective ones in the base graph. Previous versions of this class
 * enforced such identity, at a severe performance cost. Currently it is no longer enforced. If you
 * want to achieve a "live-window" functionality, your safest tactics would be to NOT override the
 ...
 *
 * <p>This graph implementation guarantees deterministic vertex and edge set ordering (via {@link
 * LinkedHashSet}).
 *
 * <p>Note that this implementation tries to maintain a "live-window" on the base graph, which has
...
public class AsSubgraph<V, E> extends AbstractGraph<V, E> implements Serializable {
   ...
   *     are included.
   * @param edgeSubset edges to in include in the subgraph. If <code>null</code> then all the edges
  ...
  /**
   * Creates a new induced subgraph. The subgraph will keep track of edges being added to its vertex
   * subset as well as deletion of edges and vertices. If base it not listenable, this is identical
   * to the call Subgraph(base, vertexSubset, null).
   *
   * @param base the base (backing) graph on which the subgraph will be based.
   * @param vertexSubset vertices to include in the subgraph. If <code>null</code> then all vertices
   *     are included.
  ...
  /**
   * Add an edge to the subgraph. The end-points must exist in the subgraph and the edge must exist
   * in the base graph. In case multiple such edges exist in the base graph, one that is not already
   * in the subgraph is chosen arbitrarily and added to the subgraph. In case all such edges already
   * exist in the subgraph, the method returns null.
   *
   ...
   *     subgraph
   * @throws IllegalArgumentException if the source or target vertex does not belong to the subgraph
   ...
   * {@inheritDoc}
   *
   * <p>By default this method returns the sum of in-degree and out-degree. The exact value returned
  ...
  private void initialize(Set<? extends V> vertexFilter, Set<? extends E> edgeFilter) {
    ...
    if (vertexFilter == null) {
      ...
      if (vertexFilter.size() > base.vertexSet().size()) {
        base.vertexSet()
            .stream()
            .filter(v -> vertexFilter.contains(v))
            .forEach(v -> vertexSet.add(v));
      } else {
        vertexFilter
            .stream()
            .filter(v -> v != null && base.containsVertex(v))
            .forEach(v -> vertexSet.add(v));
      }
    }
