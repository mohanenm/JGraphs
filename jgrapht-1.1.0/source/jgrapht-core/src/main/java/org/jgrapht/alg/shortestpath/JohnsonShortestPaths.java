 ...
 *
 * <p>Finds the shortest paths between all pairs of vertices in a sparse graph. Edge weights can be
 ...
 *
 * <p>Running time is $O(n m + n^2 \log n)$.
 *
 * <p>Since Johnson's algorithm creates additional vertices, this implementation requires the user
 * to provide a {@link VertexFactory}. Since the graph already contains vertices, care must be taken
 * so that the provided vertex factory does not return nodes that are already contained in the
...
public class JohnsonShortestPaths<V, E> extends BaseShortestPathAlgorithm<V, E> {
   ...
   * {@inheritDoc}
   *
   * @throws IllegalArgumentException in case the provided vertex factory creates vertices which are
  ...
  private void run() {
    ...
    } else runWithPositiveEdgeWeights(graph);
  }
  ...
  private void runWithNegativeEdgeWeights(Graph<V, E> g) {
    ...
    for (V v : g.vertexSet()) {
      ...
      DijkstraClosestFirstIterator<V, E> it =
          new DijkstraClosestFirstIterator<>(newEdgeWeightsGraph, v, Double.POSITIVE_INFINITY);
      while (it.hasNext()) {
        it.next();
      }
