 ...
 *
 * <p>The inspector methods work in a lazy fashion: no computation is performed unless immediately
 * necessary. Computation are done once and results and cached within this class for future need.
 *
 * <p>The inspector is also a {@link org.jgrapht.event.GraphListener}. If added as a listener to the
...
public class ConnectivityInspector<V, E> implements GraphListener<V, E> {
   ...
   * specified vertex. For more on maximally connected component, see <a
  ...
  /**
   * Returns a list of <code>Set</code> s, where each set contains all vertices that are in the same
   * maximally connected component. All graph vertices occur in exactly one set. For more on
   * maximally connected component, see <a
   * href="http://www.nist.gov/dads/HTML/maximallyConnectedComponent.html">
   * http://www.nist.gov/dads/HTML/maximallyConnectedComponent.html</a>.
   *
   * @return Returns a list of <code>Set</code> s, where each set contains all vertices that are in
  ...
  /**
   * Tests if there is a path from the specified source vertex to the specified target vertices. For
   * a directed graph, direction is ignored for this interpretation of path.
   *
   * <p>Note: Future versions of this method might not ignore edge directions for directed graphs.
   *
