 ...
 *
 * <p>The algorithm is a variant of the Bellman-Ford algorithm but instead of only storing the best
 ...
 * @since July 5, 2007
 */
public class KShortestPaths<V, E> implements KShortestPathAlgorithm<V, E> {
   ...
   * used in addition to the basic path validations which are that the path is from start to target
   * with no loops.
   *
   * @param graph graph on which shortest paths are searched.
   * @param k number of paths to be computed.
  ...
  /**
   * Constructs an object to calculate ranking shortest paths in a graph. A non-null path validator
   * may be used to accept/deny paths according to some external logic. These validations will be
   * used in addition to the basic path validations which are that the path is from start to target
   * with no loops.
   *
   ...
   * @throws IllegalArgumentException if the graph does not contain the startVertex or the endVertex
