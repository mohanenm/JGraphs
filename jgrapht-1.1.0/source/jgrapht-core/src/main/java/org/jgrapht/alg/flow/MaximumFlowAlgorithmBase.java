 ...
 * href="https://en.wikipedia.org/wiki/Maximum_flow_problem">maximum-flow</a> from the supplied <a
 * href="https://en.wikipedia.org/wiki/Flow_network">flow network</a>
 *
...
public abstract class MaximumFlowAlgorithmBase<V, E>
   ...
   * direction (v,u), and f1 be the desired increase of flow in direction (u,v). If {@literal f1 >=
   * f2}, then the flow on (v,u) becomes 0, and the flow on (u,v) becomes f1-f2. Else, if {@literal
   * f1<f2}, the flow in the direction (v,u) is reduced, i.e. the flow on (v,u) becomes f2-f1,
   * whereas the flow on (u,v) remains zero.
   *
  ...
  class AnnotatedFlowEdge implements Extension {
    ...
    public String toString() {
      return "("
          + (source == null ? null : source.prototype)
          + ","
          + (target == null ? null : target.prototype)
          + ",c:"
          + capacity
          + " f: "
          + flow
  ...
  /**
   * Returns current source vertex, or <tt>null</tt> if there was no <tt> calculateMaximumFlow</tt>
   * calls.
   *
  ...
  /**
   * Returns current sink vertex, or <tt>null</tt> if there was no <tt> calculateMaximumFlow</tt>
   * calls.
   *
   * @return current sink
   */
  ...
  /**
   * Returns maximum flow value, that was calculated during last <tt> calculateMaximumFlow</tt>
   * call.
   *
   ...
   * undirected graph, flow may go through the edge in either side. If the flow goes from u to v, we
   * return v, otherwise u. If the flow on an edge equals 0, the returned value has no meaning.
   *
   * @param e edge
  ...
  public V getFlowDirection(E e) {
    ...
    if (directedGraph) return annotatedFlowEdge.getTarget().prototype;

    AnnotatedFlowEdge inverseEdge = annotatedFlowEdge.getInverse();
    if (annotatedFlowEdge.flow > inverseEdge.flow) return annotatedFlowEdge.getTarget().prototype;
    else return inverseEdge.getTarget().prototype;
  }
  ...
  public Set<V> getSourcePartition() {
    if (sourcePartition == null) calculateSourcePartition();
  ...
  public Set<E> getCutEdges() {
    if (cutEdges != null) return cutEdges;
    ...
    if (directedGraph) {
      ...
      cutEdges.addAll(
          network
              ...
              .filter(
                  e ->
                      p1.contains(network.getEdgeSource(e)) ^ p1.contains(network.getEdgeTarget(e)))
              .collect(Collectors.toList()));
    }
    return cutEdges;
  }
  ...
  protected void calculateSourcePartition() {
    ...
    while (!processQueue.isEmpty()) {
      ...
      if (sourcePartition.contains(vx.prototype)) continue;
      sourcePartition.add(vx.prototype);
      for (AnnotatedFlowEdge ex : vx.getOutgoing()) {
        if (ex.hasCapacity()) processQueue.add(ex.getTarget());
      }
    }
  }
}
