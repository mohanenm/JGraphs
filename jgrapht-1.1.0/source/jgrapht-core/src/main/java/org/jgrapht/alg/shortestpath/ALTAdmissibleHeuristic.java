 ...
 *
 * <p>The heuristic requires a set of input nodes from the graph, which are used as landmarks.
 * During a pre-processing phase, which requires two shortest path computations per landmark using
 * Dijkstra's algorithm, all distances to and from these landmark nodes are computed and stored.
 * Afterwards, the heuristic estimates the distance from a vertex to another vertex using the
 * already computed distances to and from the landmarks and the fact that shortest path distances
 * obey the triangle-inequality. The heuristic's space requirement is O(n) per landmark where n is
 * the number of vertices of the graph. In case of undirected graphs only one Dijkstra's algorithm
 * execution is performed per landmark.
 *
 * <p>The method generally abbreviated as ALT (from A*, Landmarks and Triangle inequality) is
 * described in detail in the following <a href=
 ...
 *       (SODA' 05), 156--165, 2005.
 * </ul>
 *
 * <p>Note that using this heuristic does not require the edge weights to satisfy the
 ...
 * not be present.
 *
 * <p>In general more landmarks will speed up A* but will need more space. Given an A* query with
