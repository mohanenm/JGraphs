...
/**
 * A graph backed by the the graph specified at the constructor, which can be listened by <code>
 * GraphListener</code> s and by <code>
 ...
 * graph, but relies on <tt>Object</tt>'s <tt>equals</tt> and <tt>hashCode</tt> methods.
 *
...
public class DefaultListenableGraph<V, E> extends GraphDelegator<V, E>
  ...
  /**
   * Creates a new listenable graph. If the <code>reuseEvents</code> flag is set to <code>true
   * </code> this class will reuse previously fired events and will not create a new object for each
   * event. This option increases performance but should be used with care, especially in
   ...
   * previously fired events and will not create a new object for each event. This option increases
   * performance but should be used with care, especially in multithreaded environment.
   *
   * @param reuseEvents whether to reuse previously fired event objects instead of creating a new
   *     event object for each event.
   */
  ...
  /**
   * Tests whether the <code>reuseEvents</code> flag is set. If the flag is set to <code>true</code>
   * this class will reuse previously fired events and will not create a new object for each event.
