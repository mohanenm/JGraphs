...
final class RankingPathElementList<V, E>
  ...
  /**
   * Adds paths in the list at vertex y. Candidate paths are obtained by concatenating the specified
   * edge (v->y) to the paths <code>
   * elementList</code> at vertex v. Complexity =
   *
   * <ul>
   ...
   *       the list, <code>m</code> is the number of edges of the graph and <code>n</code> is the
   *       number of vertices of the graph, O(<code>m+n</code>) being the complexity of the <code>
   * ConnectivityInspector</code> to check whether a path exists towards the guard-vertex
   * </ul>
   *
  ...
  public boolean addPathElements(RankingPathElementList<V, E> elementList, E edge) {
    assert (this.vertex.equals(
        Graphs.getOppositeVertex(this.graph, edge, elementList.getVertex())));
   ...
   *
   * @return <code>true</code> if the specified path element disconnects the guard-vertex, <code>
  ...
  private boolean isGuardVertexDisconnected(RankingPathElement<V, E> prevPathElement) {
    ...
    MaskSubgraph<V, E> connectivityGraph =
        new MaskSubgraph<>(
            this.graph,
            v -> connectivityMask.isVertexMasked(v),
