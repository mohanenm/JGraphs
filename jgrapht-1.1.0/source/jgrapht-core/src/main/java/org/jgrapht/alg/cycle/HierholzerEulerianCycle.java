 ...
 * the graph.
 *
 * <p>See the <a href="https://en.wikipedia.org/wiki/Eulerian_path">Wikipedia article</a> for
 * details and references about Eulerian cycles and a short description of Hierholzer's algorithm
 * for the construction of an Eulerian cycle. The original presentation of the algorithm dates back
 * to 1873 and the following paper: Carl Hierholzer: &Uuml;ber die M&ouml;glichkeit, einen Linienzug
 * ohne Wiederholung und ohne Unterbrechung zu umfahren. Mathematische Annalen 6(1), 30–32, 1873.
 *
...
public class HierholzerEulerianCycle<V, E> implements EulerianCycleAlgorithm<V, E> {
  ...
  /**
   * Test whether a graph is Eulerian. An <a
  ...
  public boolean isEulerian(Graph<V, E> graph) {
    ...
    if (graph.vertexSet().isEmpty()) {
      ...
      boolean foundComponentWithEdges = false;
      for (Set<V> component :
          new KosarajuStrongConnectivityInspector<V, E>(graph).stronglyConnectedSets()) {
        for (V v : component) {
   ...
   * locations for vertices with non-zero degrees. It is important to move vertices with new insert
   * locations to the front of the vertex list, in order to make sure that we always start partial
   * cycles from already visited vertices.
   *
