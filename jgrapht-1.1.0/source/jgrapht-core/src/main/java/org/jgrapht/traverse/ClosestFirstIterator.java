...
public class ClosestFirstIterator<V, E>
   ...
   * component which includes that vertex and is reachable via paths of weighted length less than or
   * equal to the specified radius. The specified start vertex may not be <code>
   * null</code>.
   *
   * @param g the graph to be iterated.
   * @param startVertex the vertex iteration to be started.
   ...
   * requested vertex. If the vertex has already been visited, then it is truly the minimum spanning
   * tree edge; otherwise, it is the best candidate seen so far.
   *
  ...
  /**
   * Determine weighted path length to a vertex via an edge, using the path length for the opposite
   * vertex.
   *
