 ...
 * walk is a connected sequence of vertices and edges in a graph {@code v0, e0, v1, e1, v2,....vk-1,
 * ek-1, vk}, such that for {@code 1<=i<=k<}, the edge {@code e_i} has endpoints {@code v_(i-1)} and
 * {@code v_i}. The class makes no assumptions with respect to the shape of the walk: edges may be
 * repeated, and the start and end point of the walk may be different.
 *
 * <p>See <a
 * href="http://mathworld.wolfram.com/Walk.html">http://mathworld.wolfram.com/Walk.html</a>
 * GraphWalk is the default implementation of {@link GraphPath}.
 *
 * <p>Two special cases exist:
 *
 * <ol>
 *   <li>A singleton GraphWalk has an empty edge list (the length of the path equals 0), the vertex
 *       list contains a single vertex v, and the start and end vertex equal v.
 *   <li>An empty Graphwalk has empty edge and vertex lists, and the start and end vertex are both
 *       null.
 * </ol>
 *
 * <p>This class is implemented as a light-weight data structure; this class does not verify whether
...
public class GraphWalk<V, E> implements GraphPath<V, E> {
  ...
  /**
   * Creates a walk defined by a sequence of edges. A walk defined by its edges can be specified for
   * non-simple graphs. Edge repetition is permitted, the start and end point points (v0 and vk) can
   * be different.
   *
   ...
   * verified during the construction of the walk. This constructor makes it possible to store both
   * a vertex and an edge view of the same walk, thereby saving computational overhead when
  ...
  public int getLength() {
    if (edgeList != null) return edgeList.size();
    else if (vertexList != null && !vertexList.isEmpty()) return vertexList.size() - 1;
    else return 0;
  }

  @Override
  public String toString() {
    if (vertexList != null) return vertexList.toString();
    else return edgeList.toString();
  }

  @Override
  public boolean equals(Object o) {
    if (o == null || !(o instanceof GraphWalk)) return false;
    else if (this == o) return true;
    @SuppressWarnings("unchecked")
    GraphWalk<V, E> other = (GraphWalk<V, E>) o;
    if (this.isEmpty() && other.isEmpty()) return true;
    if (!this.startVertex.equals(other.getStartVertex())
        || !this.endVertex.equals(other.getEndVertex())) return false;
  ...
  public int hashCode() {
    ...
    if (edgeList != null) return 31 * hashCode + edgeList.hashCode();
    else return 31 * hashCode + vertexList.hashCode();
  }
   ...
   * is not present in the graph. The weight of the resulting walk equals the sum of edge weights in
   * the walk.
   *
  ...
  public GraphWalk<V, E> reverse(Function<GraphWalk<V, E>, Double> walkWeightCalculator) {
    ...
    if (vertexList != null) {
      ...
      if (graph.getType().isUndirected()) revWeight = this.weight;
          ...
          if (edge == null)
            throw new InvalidGraphWalkException(
                "this walk cannot be reversed. The graph does not contain a reverse arc for arc "
                    + graph.getEdge(v, u));
          else revWeight += graph.getEdgeWeight(edge);
        }
      }
    }
    ...
    if (walkWeightCalculator == null) gw.weight = revWeight;
    else gw.weight = walkWeightCalculator.apply(gw);
    return gw;
  }

  /**
   * Concatenates the specified GraphWalk to the end of this GraphWalk. This action can only be
   * performed if the end vertex of this GraphWalk is the same as the start vertex of the extending
   * GraphWalk
   *
  ...
  public GraphWalk<V, E> concat(
    ...
    if (this.isEmpty()) throw new IllegalArgumentException("An empty path cannot be extended");
  ...
  /**
   * Returns true if the path is an empty path, that is, a path with startVertex=endVertex=null and
   * with an empty vertex and edge list.
   *
