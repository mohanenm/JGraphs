 ...
 *
 * <p>The cache also keeps track of successors and predecessors for each vertex. This means that the
...
public class NeighborCache<V, E> implements GraphListener<V, E> {
  ...
  /**
   * Returns the unique predecessors of the given vertex if it exists in the cache, otherwise it is
   * initialized.
   *
   ...
   * duplicating entries in the neighbor set. It is thus more efficient to use {@link #neighborsOf}
