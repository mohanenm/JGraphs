 ...
 * Graphs</a> and the <a
 * href="https://en.wikipedia.org/wiki/Erd%C5%91s%E2%80%93R%C3%A9nyi_model">Erdős–Rényi model</a> .
 *
 * <p>In the G(n, M) model, a graph is chosen uniformly at random from the collection of all graphs
 ...
 *
 * <p>The implementation creates the vertices and then randomly chooses an edge and tries to add it.
 * If the add fails for any reason (an edge already exists and multiple edges are not allowed) it
 * will just choose another and try again. The performance therefore varies significantly based on
 * the probability of successfully constructing an acceptable edge.
 *
 * <p>The implementation tries to guess the number of allowed edges based on the following. If
 * self-loops or multiple edges are allowed and requested, the maximum number of edges is {@link
 * Integer#MAX_VALUE}. Otherwise the maximum for undirected graphs with n vertices is n(n-1)/2 while
 * for directed n(n-1). If the graph type cannot be determined (for example using adapter classes or
 * user-created custom graph types) the generator assumes the graph is undirected and therefore uses
 * n(n-1)/2 as the maximum number of edges. If the user requests self-loops and/or multiple edges
 * and the graph type cannot be determined, the corresponding feature is silently ignored.
 *
 * <p>For the G(n, p) model please see {@link GnpRandomGraphGenerator}.
 *
 * @author Assaf Lehr
