 ...
 *
 * <p>The <a href="http://en.wikipedia.org/wiki/Floyd-Warshall_algorithm">Floyd-Warshall
 * algorithm</a> finds all shortest paths (all $n^2$ of them) in $O(n^3)$ time. Note that during
 * construction time, no computations are performed! All computations are performed the first time
 * one of the member methods of this class is invoked. The results are stored, so all subsequent
 * calls to the same method are computationally efficient.
 *
...
public class FloydWarshallShortestPaths<V, E> extends BaseShortestPathAlgorithm<V, E> {
   ...
   * returned, etc. This method is computationally cheaper than calling {@link #getPath(Object,
   * Object)} and then reading the first vertex.
   *
   * @param a source vertex
   * @param b target vertex
   * @return next hop on the shortest path from a to b, or null when there exists no path from a to
   *     b.
   */
   ...
   * time is O(1). If the shortest path from a to b is a,c,d,e,b, this method returns e. If the next
   * invocation would query the next hop on the shortest path from c to e, vertex d would be
   * returned, etc. This method is computationally cheaper than calling {@link #getPath(Object,
   * Object)} and then reading the vertex. The first invocation of this method populates a last hop
   * matrix.
   *
   * @param a source vertex
   * @param b target vertex
   * @return last hop on the shortest path from a to b, or null when there exists no path from a to
   *     b.
   */
  ...
  private void populateLastHopMatrix() {
    ...
    for (int i = 0; i < n; i++) {
      for (int j = 0; j < n; j++) {
        if (i == j || lastHopMatrix[i][j] != null || backtrace[i][j] == null) continue;
