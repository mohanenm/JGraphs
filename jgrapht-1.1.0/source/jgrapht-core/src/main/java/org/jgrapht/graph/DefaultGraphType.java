 ...
 *
 * <p>The graph type describes various properties of a graph such as whether it is directed,
 * undirected or mixed, whether it contain self-loops (edges with the same source and target
 * vertices), whether it contain parallel-edges (multiple edges with the same source and target) and
...
public class DefaultGraphType implements GraphType, Serializable {
  ...
  /**
   * A simple graph type. An undirected graph for which at most one edge connects any two vertices,
   * and self-loops are not permitted.
   *
  ...
  /**
   * A directed multigraph type. A non-simple undirected graph in which no self-loops are permitted,
