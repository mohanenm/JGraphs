 ...
 *
 * <p>The user provides the sizes n1 and n2 of the two partitions (n1+n2=n) and a number m which is
 * the total number of edges to create. The generator supports both directed and undirected graphs.
 *
...
public class GnmRandomBipartiteGraphGenerator<V, E> implements GraphGenerator<V, E, V> {
  ...
  /**
   * Create a new random bipartite graph generator. The generator uses the G(n, m) model when n = n1
   * + n2 and the bipartite graph has one partition with size n1 and one partition with size n2. In
   * this model a graph is chosen uniformly at random from the collection of bipartite graphs whose
   * partitions have sizes n1 and n2 respectively and m edges.
   *
  ...
  public void generateGraph(
    ...
    if (m > maxAllowedEdges) {
      throw new IllegalArgumentException(
          "number of edges not valid for bipartite graph with " + n1 + " and " + n2 + " vertices");
    }
  ...
  /**
   * Returns the first partition of vertices in the bipartite graph. This partition is guaranteed to
   * be smaller than or equal in size to the second partition.
   *
   * @return one partition of the bipartite graph
   */
  public Set<V> getFirstPartition() {
    if (partitionA.size() <= partitionB.size()) return new LinkedHashSet<>(partitionA.values());
    else return new LinkedHashSet<>(partitionB.values());
  }

  /**
   * Returns the second partitions of vertices in the bipartite graph. This partition is guaranteed
   * to be larger than or equal in size to the first partition.
   *
   * @return one partition of the bipartite graph
   */
  public Set<V> getSecondPartition() {
    if (partitionB.size() >= partitionA.size()) return new LinkedHashSet<>(partitionB.values());
    else return new LinkedHashSet<>(partitionA.values());
  }
}
