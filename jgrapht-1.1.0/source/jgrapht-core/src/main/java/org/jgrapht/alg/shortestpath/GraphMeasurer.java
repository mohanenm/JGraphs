 ...
 * distance metrics can be found <a
 * href="https://en.wikipedia.org/wiki/Distance_(graph_theory)">here</a>.
 *
...
public class GraphMeasurer<V, E> {
   ...
   *
   * @param graph input graph
   * @param shortestPathAlgorithm shortest path algorithm used to compute shortest paths between all
   *     pairs of vertices. Recommended algorithms are: {@link
   *     org.jgrapht.alg.shortestpath.JohnsonShortestPaths} (Runtime complexity: $O(|V||E| + |V|^2
   *     log|V|)$) or {@link org.jgrapht.alg.shortestpath.FloydWarshallShortestPaths} (Runtime
   *     complexity: $O(|V|^3)$.
   */
   ...
   * where $d(u,v)$ is the shortest path between vertices $u$ and $v$. If the graph is disconnected,
   * the eccentricity of each vertex is {@link Double#POSITIVE_INFINITY}. The runtime complexity of
   * this method is $O(n^2+L)$, where $L$ is the runtime complexity of the shortest path algorithm
  ...
  public Set<V> getGraphCenter() {
    ...
    for (Map.Entry<V, Double> entry : eccentricityMap.entrySet()) {
      if (comp.compare(entry.getValue(), radius) == 0) graphCenter.add(entry.getKey());
    }
    return graphCenter;
  }

  /**
   * Compute the <a href="http://mathworld.wolfram.com/GraphPeriphery.html">graph periphery</a>. The
   * periphery of a graph is the set of vertices of graph eccentricity equal to the graph diameter.
   *
  ...
  public Set<V> getGraphPeriphery() {
    ...
    ToleranceDoubleComparator comp = new ToleranceDoubleComparator(0.000000001);
    for (Map.Entry<V, Double> entry : eccentricityMap.entrySet()) {
      if (comp.compare(entry.getValue(), diameter) == 0) graphPeriphery.add(entry.getKey());
    }
  ...
  private void computeEccentricityMap() {
    if (eccentricityMap != null) return;
