 ...
 *
 * <p>The metric for <i>closest</i> here is the weighted path length from a start vertex, i.e.
 ...
 *
 * <p>NOTE: This is an internal iterator for use in shortest paths algorithms. For an iterator that
 * is suitable to return to the users see {@link org.jgrapht.traverse.ClosestFirstIterator}. This
...
class DijkstraClosestFirstIterator<V, E> implements Iterator<V> {
   ...
   * returned (settled) by the iterator might be included with the following properties: the
   * distance will be an upper bound on the actual shortest path and the distance will be inside the
   ...
   * for each vertex (a) the distance of the path from the source vertex and (b) the last edge used
   * to reach the vertex from the source vertex.
   *
   * <p>Only the paths to vertices which are already returned by the iterator will be shortest
   * paths. Additional paths to vertices which are not yet returned (settled) by the iterator might
   * be included with the following properties: the distance will be an upper bound on the actual
