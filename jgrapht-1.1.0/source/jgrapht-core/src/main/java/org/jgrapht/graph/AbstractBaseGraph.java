 ...
 * LinkedHashMap} and {@link LinkedHashSet}).
 *
...
public abstract class AbstractBaseGraph<V, E> extends AbstractGraph<V, E>
  ...
  protected AbstractBaseGraph(
    ...
    this.weighted = false;
    this.intrusiveEdgesSpecifics =
        Objects.requireNonNull(
            createIntrusiveEdgesSpecifics(false), GRAPH_SPECIFICS_MUST_NOT_BE_NULL);
  }
  ...
  /**
   * Returns <code>true</code> if and only if multiple edges are allowed in this graph. The meaning
   * of multiple edges is that there can be many edges going from vertex v1 to vertex v2.
   *
  ...
  public GraphType getType() {
    if (directed) {
      return new DefaultGraphType.Builder()
          .directed()
          .weighted(weighted)
          .allowMultipleEdges(allowingMultipleEdges)
          .allowSelfLoops(allowingLoops)
          .build();
    } else {
      return new DefaultGraphType.Builder()
          .undirected()
  ...
  /**
   * Create the specifics for this graph. Subclasses can override this method in order to adjust the
   * specifics and thus the space-time tradeoffs of the graph implementation.
   *
   ...
   * specifics and thus the space-time tradeoffs of the graph implementation.
   *
   * @param directed if true the specifics should adjust the behavior to a directed graph otherwise
   *     undirected
   * @return the specifics used by this graph
   */
