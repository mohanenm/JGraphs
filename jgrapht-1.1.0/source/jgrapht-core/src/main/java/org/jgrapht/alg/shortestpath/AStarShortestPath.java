 ...
 * A* shortest path.
 *
 * <p>An implementation of <a href="http://en.wikipedia.org/wiki/A*_search_algorithm">A* shortest
 * path algorithm</a>. This class works for directed and undirected graphs, as well as multi-graphs
 * and mixed-graphs. The graph can also change between invocations of the {@link #getPath(Object,
 * Object)} method; no new instance of this class has to be created. The heuristic is implemented
 * using a FibonacciHeap data structure to maintain the set of open nodes. However, there still
 * exist several approaches in literature to improve the performance of this heuristic which one
 * could consider to implement. Another issue to take into consideration is the following: given two
 * candidate nodes, i, j to expand, where f(i)=f(j), g(i)&gt;g(j), h(i)&lt;g(j), f(i)=g(i)+h(i),
 * g(i) is the actual distance from the source node to i, h(i) is the estimated distance from i to
 * the target node. Usually a depth-first search is desired, so ideally we would expand node i
 * first. Using the FibonacciHeap, this is not necessarily the case though. This could be improved
 * in a later version.
 *
 * <p>Note: This implementation works with both consistent and inconsistent admissible heuristics.
 * For details on consistency, refer to the description of the method {@link
 * #isConsistentHeuristic(AStarAdmissibleHeuristic)}. However, this class is <i>not</i> optimized
...
public class AStarShortestPath<V, E> extends BaseShortestPathAlgorithm<V, E> {
   ...
   * @param admissibleHeuristic admissible heuristic which estimates the distance from a node to the
   *     target node. The heuristic must never overestimate the distance.
   */
  ...
  /**
   * Returns how many nodes have been expanded in the A* search procedure in its last invocation. A
   * node is expanded if it is removed from the open list.
   *
  ...
  /**
   * Returns true if the provided heuristic is a <i>consistent</i> or <i>monotone</i> heuristic wrt
   * the graph provided at construction time. A heuristic is monotonic if its estimate is always
   * less than or equal to the estimated distance from any neighboring vertex to the goal, plus the
   * step cost of reaching that neighbor. For details, refer to <a href=
  ...
  public boolean isConsistentHeuristic(AStarAdmissibleHeuristic<V> admissibleHeuristic) {
    for (V targetVertex : graph.vertexSet()) {
      for (E e : graph.edgeSet()) {
        ...
        if (h_x > weight + h_y) return false;
      }
    }
    return true;
  }
