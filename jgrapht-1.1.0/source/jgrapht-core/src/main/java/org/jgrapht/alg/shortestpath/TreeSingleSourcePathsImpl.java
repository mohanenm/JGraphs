 ...
 *
 * <p>This implementation uses the traditional representation of maintaining for each vertex the
 * predecessor in the shortest path tree. In order to keep space to linear, the paths are recomputed
 * in each invocation of the {@link #getPath(Object)} method. The complexity of {@link
 * #getPath(Object)} is linear to the number of edges of the path while the complexity of {@link
 * #getWeight(Object)} is O(1).
 *
...
public class TreeSingleSourcePathsImpl<V, E> implements SingleSourcePaths<V, E>, Serializable {
   ...
   * @param distanceAndPredecessorMap a map which contains for each vertex the distance and the last
   *     edge that was used to discover the vertex. The map does not need to contain any entry for
   *     the source vertex. In case it does contain the predecessor at the source vertex must be
   *     null.
   */
  public TreeSingleSourcePathsImpl(
    ...
    this.map =
        Objects.requireNonNull(distanceAndPredecessorMap, "Distance and predecessor map is null");
  }
