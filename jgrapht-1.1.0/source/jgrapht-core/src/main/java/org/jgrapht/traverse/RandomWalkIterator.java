 ...
 *
 * <p>At each step the iterator selects a random (uniformly distributed) edge out of the current
 * vertex and follows it to the next vertex. In case of directed graphs the outgoing edge set is
 * used. See <a href="https://en.wikipedia.org/wiki/Random_walk#Random_walk_on_graphs">wikipedia</a>
 * for more details.
 *
 * <p>In case a weighted walk is desired, edges are selected with probability respective to its
 * weight (out of the total weight of the edges). The walk can be bounded by number of steps
 * (default {@code Long#MAX_VALUE} . When the bound is reached the iterator is considered exhausted.
 * Calling {@code next()} on exhausted iterator will throw {@code NoSuchElementException}.
 *
 * <p>In case a sink (i.e. no edges) vertex is reached, any consecutive calls to {@code next()} will
 * throw {@code NoSuchElementException}.
 *
 * <p>For this iterator to work correctly the graph must not be modified during iteration. Currently
...
public class RandomWalkIterator<V, E> extends AbstractGraphIterator<V, E> {
  ...
  /**
   * Creates a new iterator for the specified graph. Iteration will start at arbitrary vertex. Walk
   * is un-weighted and bounded by {@code Long#MAX_VALUE} steps.
   *
   * @param graph the graph to be iterated.
   * @throws IllegalArgumentException if <code>graph==null</code> or does not contain <code>
   *     startVertex</code>
   */
   ...
   * null</code>, Iteration will start at an arbitrary graph vertex. Walk is un-weighted and bounded
   * by {@code Long#MAX_VALUE} steps.
   *
   * @param graph the graph to be iterated.
   ...
   * Creates a new iterator for the specified graph. Iteration will start at the specified start
   * vertex. If the specified start vertex is <code>
   * null</code>, Iteration will start at an arbitrary graph vertex. Walk is bounded by {@code
   * Long#MAX_VALUE} steps.
   *
   * @param graph the graph to be iterated.
   * @param startVertex the vertex iteration to be started.
   ...
   * Creates a new iterator for the specified graph. Iteration will start at the specified start
   * vertex. If the specified start vertex is <code>
   * null</code>, Iteration will start at an arbitrary graph vertex. Walk is bounded by the provided
   * number steps.
   *
   * @param graph the graph to be iterated.
   * @param startVertex the vertex iteration to be started.
   ...
   * @param edge the edge via which the vertex was encountered, or null if the vertex is a starting
   *     point
   */
   ...
   * selected with uniform distribution across all outgoing edges. In case of a weighted walk, edge
   * will be selected with probability respective to its weight across all outgoing edges.
   *
