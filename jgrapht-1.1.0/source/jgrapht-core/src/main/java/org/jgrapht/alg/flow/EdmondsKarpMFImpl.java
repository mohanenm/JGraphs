...
/**
 * This class computes a maximum flow in a <a href =
 * "http://en.wikipedia.org/wiki/Flow_network">flow network</a> using <a href =
 * "http://en.wikipedia.org/wiki/Edmonds-Karp_algorithm">Edmonds-Karp algorithm</a>. Given is a
 * weighted directed or undirected graph $G(V,E)$ with vertex set $V$ and edge set $E$. Each edge
 * $e\in E$ has an associated non-negative capacity $u_e$. The maximum flow problem involves finding
 * a feasible flow from a source vertex $s$ to a sink vertex $t$ which is maximum. The amount of
 * flow $f_e$ through any edge $e$ cannot exceed capacity $u_e$. Moreover, flow conservation must
 * hold: the sum of flows entering a node must equal the sum of flows exiting that node, except for
 * the source and the sink nodes.
 *
 * <p>Mathematically, the maximum flow problem is stated as follows: \[ \begin{align} \max~&amp;
 ...
 * edges of vertex $i$.
 *
 * <p>When the input graph is undirected, an edge $(i,j)$ is treated as two directed arcs: $(i,j)$
 * and $(j,i)$. In such a case, there is the additional restriction that the flow can only go in one
 ...
 *
 * <p>The runtime complexity of this class is $O(nm^2)$, where $n$ is the number of vertices and $m$
 * the number of edges in the graph. For a more efficient algorithm, consider using {@link
 * PushRelabelMFImpl} instead.
 *
 * <p>This class can also compute minimum s-t cuts. Effectively, to compute a minimum s-t cut, the
 ...
 *
 * <p>Note: even though the algorithm accepts any kind of graph, currently only Simple directed and
...
public final class EdmondsKarpMFImpl<V, E> extends MaximumFlowAlgorithmBase<V, E> {
   ...
   * <tt> DEFAULT_EPSILON</tt> tolerance.
   *
  ...
  /**
   * Sets current source to <tt>source</tt>, current sink to <tt>sink</tt>, then calculates maximum
   * flow from <tt>source</tt> to <tt>sink</tt>. Note, that <tt>source</tt> and <tt>sink</tt> must
   * be vertices of the <tt> network</tt> passed to the constructor, and they must be different.
   *
   ...
   * Sets current source to <tt>source</tt>, current sink to <tt>sink</tt>, then calculates maximum
   * flow from <tt>source</tt> to <tt>sink</tt>. Note, that <tt>source</tt> and <tt>sink</tt> must
   * be vertices of the <tt> network</tt> passed to the constructor, and they must be different. If
  ...
  /**
   * Method which finds a path from source to sink the in the residual graph. Note that this method
   * tries to find multiple paths at once. Once a single path has been discovered, no new nodes are
   * added to the queue, but nodes which are already in the queue are fully explored. As such
   * there's a chance that multiple paths are discovered.
   */
