...
public abstract class GraphTests {
   ...
   * pair of distinct vertices is connected by a unique edge. A complete directed graph is a
   ...
   *
   * <p>This method does not performing any caching, instead recomputes everything from scratch. In
   ...
   * definition, any acyclic graph is a forest. This includes the empty graph and the class of tree
   * graphs.
   *
   * @param graph the input graph
   * @param <V> the graph vertex type
  ...
  /**
   * Test whether a graph is <a href="https://en.wikipedia.org/wiki/Overfull_graph">overfull</a>. A
   * graph is overfull if $|E|&gt;\Delta(G)\lfloor |V|/2 \rfloor$, where $\Delta(G)$ is the maximum
   * degree of the graph.
   *
   * @param graph the input graph
   * @param <V> the graph vertex type
  ...
  /**
   * Test whether an undirected graph is a <a href="https://en.wikipedia.org/wiki/Split_graph">split
   * graph</a>. A split graph is a graph in which the vertices can be partitioned into a clique and
   * an independent set. Split graphs are a special class of chordal graphs. Given the degree
   * sequence $d_1 \geq,\dots,\geq d_n$ of $G$, a graph is a split graph if and only if :
   * \[\sum_{i=1}^m d_i = m (m - 1) + \sum_{i=m + 1}^nd_i\], where $m = \max_i \{d_i\geq i-1\}$. If
   * the graph is a split graph, then the $m$ vertices with the largest degrees form a maximum
   * clique in $G$, and the remaining vertices constitute an independent set. See Brandstadt, A.,
   * Le, V., Spinrad, J. Graph Classes: A Survey. Philadelphia, PA: SIAM, 1999. for details.
   *
   * @param graph the input graph
   * @param <V> the graph vertex type
   * @param <E> the graph edge type
  ...
  public static <V, E> boolean isSplit(Graph<V, E> graph) {
    ...
    if (!isSimple(graph) || graph.vertexSet().isEmpty()) return false;
    ...
    int left = 0;
    for (int i = 0; i <= m; i++) left += degrees.get(i);
    int right = m * (m + 1);
    for (int i = m + 1; i < degrees.size(); i++) right += degrees.get(i);
    return left == right;
  }
  ...
  public static <V, E> boolean isBipartite(Graph<V, E> graph) {
    ...
    try {
      // at most n^2/4 edges
      if (Math.multiplyExact(4, graph.edgeSet().size())
          > Math.multiplyExact(graph.vertexSet().size(), graph.vertexSet().size())) {
        return false;
      }
  ...
  public static <V, E> boolean isCubic(Graph<V, E> graph) {
    for (V v : graph.vertexSet()) if (graph.degreeOf(v) != 3) return false;
    return true;
  }
   ...
   * vertex has the same incoming and outgoing degree. Test whether a graph is Eulerian. An <a
   * href="http://mathworld.wolfram.com/EulerianGraph.html">Eulerian graph</a> is a graph containing
   * an <a href="http://mathworld.wolfram.com/EulerianCycle.html">Eulerian cycle</a>.
   *
   * @param graph the input graph
   * @param <V> the graph vertex type
   * @param <E> the graph edge type
  ...
  /**
   * Checks that the specified graph is directed and throws a customized {@link
   * IllegalArgumentException} if it is not. Also checks that the graph reference is not {@code
   * null} and throws a {@link NullPointerException} if it is.
   *
  ...
  public static <V, E> Graph<V, E> requireDirected(Graph<V, E> graph, String message) {
    if (graph == null) throw new NullPointerException(GRAPH_CANNOT_BE_NULL);
    if (!graph.getType().isDirected()) {
   ...
   * it is not. Also checks that the graph reference is not {@code null} and throws a {@link
  ...
  /**
   * Checks that the specified graph is undirected and throws a customized {@link
   * IllegalArgumentException} if it is not. Also checks that the graph reference is not {@code
   * null} and throws a {@link NullPointerException} if it is.
   *
  ...
  /**
   * Checks that the specified graph is undirected and throws an {@link IllegalArgumentException} if
   * it is not. Also checks that the graph reference is not {@code null} and throws a {@link
   * NullPointerException} if it is.
   *
   * @param graph the graph reference to check for being undirected and not null
  ...
  /**
   * Checks that the specified graph is directed or undirected and throws a customized {@link
   * IllegalArgumentException} if it is not. Also checks that the graph reference is not {@code
   * null} and throws a {@link NullPointerException} if it is.
   *
