 ...
 *
 * <p>Find the LCA, defined as <i>Let G = (V, E) be a DAG, and let x, y ∈ V . Let G x,y be the
 * subgraph of G induced by the set of all common ancestors of x and y. Define SLCA (x, y) to be the
 * set of out-degree 0 nodes (leafs) in G x,y . The lowest common ancestors of x and y are the
 * elements of SLCA (x, y). This naive algorithm simply starts at a and b, recursing upwards to the
 * root(s) of the DAG. Wherever the recursion paths cross we have found our LCA.</i> from
 ...
 * continues
 *
 * <p>This means at the end of recursion n, we know if there is an LCA that has a path of &lt;=n to
 * a and b. Of course we may have to wait longer if the path to a is of length n, but the path to
...
public class NaiveLcaFinder<V, E> {
  ...
  /**
   * Return a single vertex that is both in x and y. If there is more than one then select the first
   * element from the iterator returned from y, after all the elements of x have been removed. this
   * allows an orderedSet to be passed in to give predictable results.
   *
