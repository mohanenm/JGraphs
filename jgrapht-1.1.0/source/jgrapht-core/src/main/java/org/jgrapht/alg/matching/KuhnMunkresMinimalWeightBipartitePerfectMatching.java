 ...
 *
 * <p>Given <a href=http://en.wikipedia.org/wiki/Complete_bipartite_graph>complete bipartite
 * graph</a> G = (S, T; E), such that |S| = |T|, and each edge has <i>non-negative</i> cost <i>c(i,
...
public class KuhnMunkresMinimalWeightBipartitePerfectMatching<V, E>
  ...
  public Matching<V, E> getMatching() {
    ...
    if (graph.vertexSet().isEmpty()) {
      ...
      matching =
          new KuhnMunkresMatrixImplementation<>(graph, firstPartition, secondPartition)
              .buildMatching();
    }
  ...
  static class KuhnMunkresMatrixImplementation<V, E> {
     ...
     * cost-matrix) to the workers (designated by the rows of the cost-matrix) so that to MINIMIZE
     * total tasks-tackling costs
     *
      ...
      while (extending && (matchingSize < excessMatrix.length)) {
        ...
        for (int j = 0; j < excessMatrix.length; ++j) {
          if ((rowMatched[j] == -1) && !colsVisited[j]) {
            extending |=
                new MatchExtender(rowsVisited, colsVisited).extend(j); /* Try to extend matching */
          }
        }
    ...
    /**
     * Extends equality-graph subtracting minimal excess from all the COLUMNS UNCOVERED and adding
     * it to the all ROWS COVERED
     */
