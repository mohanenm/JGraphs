 ...
 *
 * <p>In pseudo code, the algorithm (simplified) looks like this:
 *
 * <pre>
 * <code>
 *  VC(G):
 ...
 *
 * <p>TODO JK: determine runtime complexity and add it to class description. TODO JK: run this class
...
public class RecursiveExactVCImpl<V, E> implements MinimumWeightedVertexCoverAlgorithm<V, E> {
  ...
  public VertexCover<V> getVertexCover(Graph<V, E> graph, Map<V, Double> vertexWeightMap) {
    ...
    for (int i = 0; i < vertices.size(); i++) vertexIDDictionary.put(vertices.get(i), i);
    ...
    for (int i = vertexCover.bitSetCover.nextSetBit(0);
        i >= 0 && i < N;
        i = vertexCover.bitSetCover.nextSetBit(i + 1)) verticesInCover.add(vertices.get(i));
  ...
  /**
   * Calculates a cheap upper bound on the optimum solution. Currently, we return the best solution
   * found by either the greedy heuristic, or Clarkson's 2-approximation. Neither of these 2
   * algorithms dominates the other. //TODO JK: Are there better bounding procedures?
   */
