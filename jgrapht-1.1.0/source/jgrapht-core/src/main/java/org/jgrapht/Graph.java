 ...
 * <tt>G(V,E)</tt> contains a set <tt>V</tt> of vertices and a set <tt> E</tt> of edges. Each edge
 ...
 *
 * <p>This library works best when vertices represent arbitrary objects and edges represent the
 * relationships between them. Vertex and edge instances may be shared by more than one graph.
 *
 * <p>Through generics, a graph can be typed to specific classes for vertices <code>V</code> and
 * edges <code>E&lt;T&gt;</code>. Such a graph can contain vertices of type <code>V</code> and all
 * sub-types and Edges of type <code>
 * E</code> and all sub-types.
 *
 * <p>For guidelines on vertex and edge classes, see <a
 * href="https://github.com/jgrapht/jgrapht/wiki/EqualsAndHashCode">this wiki page</a>.
 *
...
public interface Graph<V, E> {
  /**
   * Returns a set of all edges connecting source vertex to target vertex if such vertices exist in
   * this graph. If any of the vertices does not exist or is <code>null</code>, returns <code>null
   * </code>. If both vertices exist but no edges found, returns an empty set.
   *
   * <p>In undirected graphs, some of the returned edges may have their source and target vertices
   * in the opposite order. In simple graphs the returned set is either singleton set or empty set.
   *
  ...
  /**
   * Returns an edge connecting source vertex to target vertex if such vertices and such edge exist
   * in this graph. Otherwise returns <code>
   * null</code>. If any of the specified vertices is <code>null</code> returns <code>null</code>
   *
   * <p>In undirected graphs, the returned edge may have its source and target vertices in the
   * opposite order.
   *
   * @param sourceVertex source vertex of the edge.
   * @param targetVertex target vertex of the edge.
  ...
  /**
   * Returns the edge factory using which this graph creates new edges. The edge factory is defined
   * when the graph is constructed and must not be modified.
   *
   ...
   *
   * <p>The source and target vertices must already be contained in this graph. If they are not
   * found in graph IllegalArgumentException is thrown.
   *
   * <p>This method creates the new edge <code>e</code> using this graph's <code>EdgeFactory</code>.
   ...
   * formally, adds the specified edge, <code>
   * e</code>, to this graph if this graph contains no edge <code>e2</code> such that <code>
   * e2.equals(e)</code>. If this graph already contains such an edge, the call leaves this graph
   * unchanged and returns <tt>false</tt>. Some graphs do not allow edge-multiplicity. In such
   * cases, if the graph already contains an edge from the specified source to the specified target,
   * than this method does not change the graph and returns <code>
   ...
   * @throws IllegalArgumentException if source or target vertices are not found in the graph.
   * @throws ClassCastException if the specified edge is not assignment compatible with the class of
   *     edges produced by the edge factory of this graph.
   * @throws NullPointerException if any of the specified vertices is <code>
   * null</code>.
   ...
   * specified vertex, <code>v</code>, to this graph if this graph contains no vertex <code>u</code>
   * such that <code>
   * u.equals(v)</code>. If this graph already contains such vertex, the call leaves this graph
   * unchanged and returns <tt>false</tt>. In combination with the restriction on constructors, this
   * ensures that graphs never contain duplicate vertices.
   *
   ...
   * <tt>true</tt> if and only if this graph contains an edge <code>e2</code> such that <code>
   * e.equals(e2)</code>. If the specified edge is <code>null</code> returns <code>false</code>.
   *
   ...
   * <tt>true</tt> if and only if this graph contains a vertex <code>u</code> such that <code>
   * u.equals(v)</code>. If the specified vertex is <code>null</code> returns <code>false</code>.
   *
  ...
  /**
   * Returns a set of the edges contained in this graph. The set is backed by the graph, so changes
   ...
   *
   * <p>The graph implementation may maintain a particular set ordering (e.g. via {@link
   * java.util.LinkedHashSet}) for deterministic iteration, but this is not required. It is the
   ...
   *
   * <p>A degree of a vertex in an undirected graph is the number of edges touching that vertex.
   * Edges with same source and target vertices (self-loops) are counted twice.
   *
   * <p>In directed graphs this method returns the sum of the "in degree" and the "out degree".
   *
   ...
   *
   * <p>The "in degree" of a vertex in a directed graph is the number of inward directed edges from
   * that vertex. See <a href="http://mathworld.wolfram.com/Indegree.html">
   * http://mathworld.wolfram.com/Indegree.html</a>.
   *
   * <p>In the case of undirected graphs this method returns the number of edges touching the
   * vertex. Edges with same source and target vertices (self-loops) are counted twice.
   *
   * @param vertex vertex whose degree is to be calculated.
   * @return the degree of the specified vertex.
   */
   ...
   *
   * <p>The "out degree" of a vertex in a directed graph is the number of outward directed edges
   * from that vertex. See <a href="http://mathworld.wolfram.com/Outdegree.html">
   ...
   * @throws NullPointerException if the specified edge collection is <tt> null</tt>.
   ...
   * empty set. This method will either invoke the {@link #removeEdge(Object)} method, or the {@link
   ...
   * @return <tt>true</tt> if this graph changed as a result of the call
   * @throws NullPointerException if the specified vertex collection is <tt> null</tt>.
  ...
  /**
   * Removes an edge going from source vertex to target vertex, if such vertices and such edge exist
   * in this graph. Returns the edge if removed or <code>null</code> otherwise.
   *
   * @param sourceVertex source vertex of the edge.
   * @param targetVertex target vertex of the edge.
  ...
  /**
   * Removes the specified edge from the graph. Removes the specified edge from this graph if it is
   * present. More formally, removes an edge <code>
  ...
  /**
   * Removes the specified vertex from this graph including all its touching edges if present. More
   * formally, if the graph contains a vertex <code>
   * u</code> such that <code>u.equals(v)</code>, the call removes all edges that touch <code>u
   * </code> and then removes <code>u</code> itself. If no such <code>u</code> is found, the call
   * leaves the graph unchanged. Returns <tt>true</tt> if the graph contained the specified vertex.
   ...
   * changes to the graph are reflected in the set. If the graph is modified while an iteration over
  ...
  /**
   * Get the graph type. The graph type can be used to query for additional metadata such as whether
   * the graph supports directed or undirected edges, self-loops, parallel-edges, weights, etc.
   *
  ...
  /**
   * Returns the weight assigned to a given edge. Unweighted graphs return 1.0 (as defined by {@link
   * #DEFAULT_EDGE_WEIGHT}), allowing weighted-graph algorithms to apply to them when meaningful.
   *
   * @param e edge of interest
   * @return edge weight
   */
