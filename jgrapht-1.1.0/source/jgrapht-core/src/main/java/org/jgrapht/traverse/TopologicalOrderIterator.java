 ...
 *
 * <p>A topological order is a permutation <tt>p</tt> of the vertices of a graph such that an edge
 * <tt>(i,j)</tt> implies that <tt>i</tt> appears before <tt>j</tt> in <tt>p</tt>. For more
 * information see <a href="https://en.wikipedia.org/wiki/Topological_sorting">wikipedia</a> or <a
 * href="http://mathworld.wolfram.com/TopologicalSort.html">wolfram</a>.
 *
 * <p>The iterator crosses components but does not track them, it only tracks visited vertices. The
...
public class TopologicalOrderIterator<V, E> extends AbstractGraphIterator<V, E> {
   ...
   *
   * <p>Traversal will start at one of the graph's <i>sources</i>. See the definition of source at
   ...
   * Construct a topological order iterator.
   *
   * <p>Creates a new topological order iterator over the directed graph specified, with a
   * user-supplied queue implementation to allow customized control over tie-breaking in case of
   * partial order. Traversal will start at one of the graph's <i>sources</i>. See the definition of
   * source at <a href="http://mathworld.wolfram.com/Source.html">
   ...
   * <p>Traversal will start at one of the graph's <i>sources</i>. See the definition of source at
   * <a href="http://mathworld.wolfram.com/Source.html">
   * http://mathworld.wolfram.com/Source.html</a>. In case of partial order, a comparator is used to
   * break ties.
   *
   * @param graph the directed graph to be iterated
   ...
   * {@inheritDoc}
   *
   * <p>Always returns true since the iterator does not care about components.
   */
  @Override
   ...
   * {@inheritDoc}
   *
   * <p>Trying to disable the cross components nature of this iterator will result into throwing a
   * {@link IllegalArgumentException}.
   */
  @Override
