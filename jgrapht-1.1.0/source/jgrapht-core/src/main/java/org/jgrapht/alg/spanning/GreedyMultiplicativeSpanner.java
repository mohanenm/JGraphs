...
/**
 * Greedy algorithm for (2k-1)-multiplicative spanner construction (for any integer {@literal k >=
 * 1}).
 *
 * <p>The spanner is guaranteed to contain O(n^{1+1/k}) edges and the shortest path distance between
 ...
 *
 * <p>The algorithm is described in: Althoefer, Das, Dobkin, Joseph, Soares. <a
 * href="https://doi.org/10.1007/BF02189308">On Sparse Spanners of Weighted Graphs</a>. Discrete
 * Computational Geometry 9(1):81-100, 1993.
 *
 * <p>If the graph is unweighted the algorithm runs in O(m n^{1+1/k}) time. Setting k to infinity
 ...
 *
 * <p>If the graph is weighted the algorithm runs in O(m (n^{1+1/k} + nlogn)) time by using
...
public class GreedyMultiplicativeSpanner<V, E> implements SpannerAlgorithm<E> {
  ...
  private abstract class SpannerAlgorithmBase {
    ...
    public Spanner<E> run() {
      ...
      Collections.sort(
          allEdges,
          (e1, e2) -> Double.valueOf(graph.getEdgeWeight(e1)).compareTo(graph.getEdgeWeight(e2)));
  ...
  private class UnweightedSpannerAlgorithm extends SpannerAlgorithmBase {
    ...
    /**
     * Check if two vertices are reachable by a BFS in the spanner graph using only a certain number
     * of hops.
     *
     * <p>We execute this procedure repeatedly, therefore we need to keep track of what it touches
