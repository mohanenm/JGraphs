 ...
 * consider computing the maximum flows/minimum cuts manually through {@link
 * MaximumFlowAlgorithm}/{@link MinimumSTCutAlgorithm}.
 *
 * <p>The runtime complexity of this class is O((V-1)Q), where Q is the runtime complexity of the
 * algorithm used to compute s-t cuts in the graph. By default, this class uses the {@link
 ...
 *
 * <p>Note: this class performs calculations in a lazy manner. The GHT is not calculated until the
 ...
 *
 * <p>In contrast to an Equivalent Flow Tree ({@link GusfieldEquivalentFlowTree}), Gomory-Hu trees
 * also provide all minimum cuts for all pairs of vertices!
 *
...
public class GusfieldGomoryHuCutTree<V, E>
  ...
  public GusfieldGomoryHuCutTree(
    ...
    if (N < 2) throw new IllegalArgumentException("Graph must have at least 2 vertices");
    this.minimumSTCutAlgorithm = minimumSTCutAlgorithm;
    vertexList.addAll(network.vertexSet());
    for (int i = 0; i < vertexList.size(); i++) indexMap.put(vertexList.get(i), i);
  }
  ...
  private void calculateGomoryHuTree() {
    ...
    for (int s = 1; s < N; s++) {
      ...
      for (int i = 0; i < N; i++)
        if (i != s && sourcePartition.contains(vertexList.get(i)) && p[i] == t) p[i] = s;
   ...
   * unique. The edge weights represent the flow values/cut weights. This method runs in O(n) time.
   *
  ...
  /**
   * Returns rhw maximum flow value, that was calculated during the last {@link
   * #calculateMaximumFlow(Object, Object)} call.
   *
  ...
  /**
   * Calculates the minimum cut in the graph, that is, the minimum cut over all s-t pairs. The same
   * result can be obtained with the {@link org.jgrapht.alg.StoerWagnerMinimumCut} implementation.
   * After invoking this method, the source/sink partitions corresponding to the minimum cut can be
   * queried through the {@link #getSourcePartition()} and {@link #getSinkPartition()} methods.
   * After computing the Gomory-Hu Cut tree, this method runs in O(N) time.
   *
   * @return weight of the minimum cut in the graph
   */
  public double calculateMinCut() {
    if (this.gomoryHuTree == null) this.gomoryHuTree = this.getGomoryHuTree();
    DefaultWeightedEdge cheapestEdge =
        gomoryHuTree
            .edgeSet()
            .stream()
            .min(Comparator.comparing(gomoryHuTree::getEdgeWeight))
  ...
  public Set<E> getCutEdges() {
    ...
    for (E e : network.edgeSet()) {
      ...
      if (sourcePartion.contains(source) ^ sourcePartion.contains(sink)) cutEdges.add(e);
    }
    return cutEdges;
  }
}
