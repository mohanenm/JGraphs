 ...
 * an object, similarly to {@link java.lang.Integer}. An object of type <code>ModifiableInteger
 * </code> contains a single field whose type is <code>int</code>.
 *
 * <p>Unlike <code>java.lang.Integer</code>, the int value which the ModifiableInteger represents
 * can be modified. It becomes useful when used together with the collection framework. For example,
 * if you want to have a {@link java.util.List} of counters. You could use <code>Integer</code> but
 * that would have became wasteful and inefficient if you frequently had to update the counters.
 *
 * <p>WARNING: Because instances of this class are mutable, great care must be exercised if used as
...
public class ModifiableInteger extends Number implements Comparable<ModifiableInteger> {
   ...
   *
   * <p>This constructor is for the use of java.beans.XMLDecoder deserialization. The constructor is
   ...
   *     argument <code>ModifiableInteger</code>; a value less than <code>0</code> if this <code>
   *     ModifiableInteger</code> is numerically less than the argument <code>ModifiableInteger
   *     </code>; and a value greater than <code>0</code> if this <code>ModifiableInteger</code> is
   *     numerically greater than the argument <code>ModifiableInteger</code> (signed comparison).
   */
  @Override
   ...
   * true</code> if and only if the argument is not <code>null</code> and is an <code>
   * ModifiableInteger</code> object that contains the same <code>
   ...
   * returned as a string, exactly as if the integer value were given as an argument to the {@link
