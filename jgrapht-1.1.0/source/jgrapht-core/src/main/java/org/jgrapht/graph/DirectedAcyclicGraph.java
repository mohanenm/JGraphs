 ...
 *
 * <p>Implements a DAG that can be modified (vertices &amp; edges added and removed), is guaranteed
 * to remain acyclic, and provides fast topological order iteration. An attempt to add an edge which
 * would induce a cycle throws an {@link IllegalArgumentException}.
 *
 * <p>This is done using a dynamic topological sort which is based on the algorithm described in
 * "David J. Pearce &amp; Paul H. J. Kelly. A dynamic topological sort algorithm for directed
 * acyclic graphs. Journal of Experimental Algorithmics, 11, 2007." (see <a
 * href="http://www.mcs.vuw.ac.nz/~djp/files/PK-JEA07.pdf">paper</a> or <a
 * href="http://doi.acm.org/10.1145/1187436.1210590">ACM link</a> for details). The implementation
 * differs from the algorithm specified in the above paper in some ways, perhaps most notably in
 * that the topological ordering is stored by default using two hash maps, which will have some
 * effects on the runtime, but also allow for vertex addition and removal. This storage mechanism
 * can be adjusted by subclasses.
 *
 * <p>The complexity of adding a new edge in the graph depends on the number of edges incident to
 ...
 *
 * <p>This class makes no claims to thread safety, and concurrent usage from multiple threads will
...
public class DirectedAcyclicGraph<V, E> extends SimpleDirectedGraph<V, E> implements Iterable<V> {
  ...
  public GraphType getType() {
    return new DefaultGraphType.Builder()
        .directed()
        .weighted(super.getType().isWeighted())
   ...
   *
   * <p>Vertices are indexed by their topological index, to avoid using the vertex type in the
   * interface.
   *
   * @author Peter Giles
   */
  protected interface VisitedStrategy {
     ...
     * @throws UnsupportedOperationException if the implementation doesn't support (or doesn't need)
     *     clearance. For example, if the factory creates a new instance every time, it is a waste
     *     of cycles to reset the state after the search of the Affected Region is done, so an
  ...
  protected class TopoVertexMap implements TopoOrderMap<V> {
     ...
     * topological indices can be negative, and we want to do it because we can make better use of
     * space by only needing an ArrayList of size |AR|.
     *
   ...
   *
   * <p>This implementation is close to the performance of {@link VisitedArrayListImpl}, with 1/8
   * the memory usage.
   *
   * @author John V. Sichi
   */
   ...
   *
   * <p>This implementation seems to offer the best performance in most cases. It grows the internal
   * ArrayList as needed to be as large as |AR|, so it will be more memory intensive than the
   * HashSet implementation, and unlike the Array implementation, it will hold on to that memory (it
   * expands, but never contracts).
   *
   * @author Peter Giles
   */
   ...
   *
   * <p>This implementation doesn't seem to perform as well, though I can imagine circumstances
   ...
   *
   * <p>This implementation, somewhat to my surprise, is slower than the ArrayList version, probably
