...
public interface ShortestPathAlgorithm<V, E> {
  ...
  /**
   * Get the weight of the shortest path from a source vertex to a sink vertex. Returns {@link
   * Double#POSITIVE_INFINITY} if no path exists.
   *
   * @param source the source vertex
   * @param sink the sink vertex
   * @return the weight of the shortest path from a source vertex to a sink vertex, or {@link
   *     Double#POSITIVE_INFINITY} if no path exists
   */
  ...
  interface SingleSourcePaths<V, E> {
     ...
     * exists, {@link Double#POSITIVE_INFINITY} is returned. The weight of the path between a vertex
     * and itself is always zero.
     *
     * @param sink the sink vertex
     * @return the weight of the path between source and sink vertices or {@link
     *     Double#POSITIVE_INFINITY} in case no such path exists
     */
