 ...
 *
 * <p>The algorithm is due to Drake and Hougardy, described in detail in the following paper:
 *
 * <ul>
 *   <li>D.E. Drake, S. Hougardy, A Simple Approximation Algorithm for the Weighted Matching
 *       Problem, Information Processing Letters 85, 211-213, 2003.
 * </ul>
 *
 * <p>This particular implementation uses by default two additional heuristics discussed by the
 * authors which also take linear time but improve the quality of the matchings. These heuristics
 * can be disabled by calling the constructor {@link #PathGrowingWeightedMatching(Graph, boolean)}.
 ...
 *
 * <p>For a discussion on engineering approximate weighted matching algorithms see the following
 * paper:
 *
 * <ul>
...
public class PathGrowingWeightedMatching<V, E> implements MatchingAlgorithm<V, E> {
   ...
   * using {@link #DEFAULT_EPSILON} tolerance. By default two additional linear time heuristics are
   * used in order to improve the quality of the matchings.
   *
   ...
   *     approximation factor remains 1/2, in most cases the heuristics produce matchings of higher
   *     quality.
   */
   ...
   *
   * <p>The work array used in the dynamic programming algorithm is reused between invocations. In
