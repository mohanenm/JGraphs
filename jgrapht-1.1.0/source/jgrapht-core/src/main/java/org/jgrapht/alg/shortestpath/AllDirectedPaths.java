...
public class AllDirectedPaths<V, E> {
   ...
   * @param maxPathLength maximum number of edges to allow in a path (if null, all paths are
   *     considered, which may be very slow due to potentially huge output)
   * @return list of all paths from the sources to the targets containing no more than maxPathLength
   *     edges
   */
  public List<GraphPath<V, E>> getAllPaths(
   ...
   * @param simplePathsOnly if true, only search simple (non-self-intersecting) paths (if null, all
   *     edges will be considered, which may be expensive)
