 ...
 * special case that the input graph is bipartite, consider using {@link
 * HopcroftKarpMaximumCardinalityBipartiteMatching} instead.
 *
 * <p>To compute a maximum cardinality matching, at most n augmenting path computations are
 * performed. Each augmenting path computation takes O(m alpha(m,n)) time, where alpha(m,n) is an
 * inverse of the Ackerman function, n is the number of vertices, and m the number of edges. This
 * results in a total runtime complexity of O(nm alpha(m,n)). In practise, the number of augmenting
 * path computations performed is far smaller than n, since an efficient heuristic is used to
 * compute a near-optimal initial solution. This implementation is highly efficient: a maximum
 * matching in a graph of 2000 vertices and 1.5 million edges is calculated in a few milliseconds on
 * a desktop computer.<br>
 ...
 *
 * <p>Edmonds' original algorithm first appeared in Edmonds, J. Paths, trees, and flowers. Canadian
 ...
 *       Matching and Extensions to b-matching and f-factors, 2016</a>
 * </ul>
 *
 * <p>For future reference - A more efficient algorithm than the one implemented in this class
 * exists: Micali, S., Vazirani, V. An O(sqrt(n)m) algorithm for finding maximum matching in general
 ...
 *       Vazirani 'Dispelling an Old Myth about an Ancient Algorithm'</a>
 *   <li><a href="https://arxiv.org/abs/1210.4594">Vazirani, V. A Simplification of the MV Matching
 *       Algorithm and its Proof, 2013</a>
 * </ul>
 *
...
public class EdmondsMaximumCardinalityMatching<V, E> implements MatchingAlgorithm<V, E> {
   ...
   * edge [v,w] causes a blossom to form containing odd vertex x. We define bridge(x) to be [v,w] if
   * x is an ancestor of v before the blossom is formed, or [w,v] if x is an ancestor of w.
   */
  ...
  /**
   * Constructs a new instance of the algorithm. {@link GreedyMaximumCardinalityMatching} is used to
   * quickly generate a near optimal initial solution.
   *
      ...
      while (!queue.isEmpty()) {
        ...
        for (V wOrig : Graphs.neighborListOf(graph, vertices.get(v))) {
          ...
          if (even[uf.find(w)] != NIL) { // w is an even vertex
            ...
            if (!uf.inSameSet(v, w)) blossom(v, w); // Create a new blossom using bridge edge (v,w)
          }
   ...
   * The vertices encountered on this path are grouped together (union). The odd vertices are added
   * to the processing queue (odd vertices in a blossom become even) and a pointer to the bridge
   * (v,w) is stored for each odd vertex. Notice the orientation of the bridge: the first vertex of
   * the bridge returned by bridge.get(x) is always on the same side of the blossom as x.
   *
  ...
  /**
   * Compute the nearest even ancestor of even node v. If v is the root of a tree, then this method
   * returns v itself.
   *
   * @param v even vertex
  ...
  private int parent(int v) {
    ...
    if (parent == v) return v; // root of tree
  ...
  private int buildPath(int[] path, int i, int start, int end) {
    while (true) {
      ...
      // root of the tree
      if (matching.isExposed(start)) return i;

      path[i++] = matching.opposite(start); // odd vertex

      // base case
      if (path[i - 1] == end) return i;
  ...
  public Matching<V, E> getMatching() {
    this.init();
    if (initializer != null) this.warmStart(initializer);
    ...
    for (int vx = 0; vx < vertices.size(); vx++) {
      if (matching.isExposed(vx)) continue;
      V v = vertices.get(vx);
   ...
   * does not exist a different matching m' in the graph which is of larger cardinality. This method
   * is solely intended for verification purposes. Any matching returned by the {@link
   * #getMatching()} method in this class is guaranteed to be maximum.
   *
   * <p>To attest whether the matching is maximum, we use the Tutte-Berge Formula which provides a
  ...
  public boolean isMaximumMatching(Matching<V, E> matching) {
    ...
    if (matching.getEdges().size() * 2 >= graph.vertexSet().size() - 1) return true;
    ...
    Set<V> oddVertices =
        vertexIndexMap
            .values()
            .stream()
            .filter(vx -> odd[vx] != NIL && !bridges.containsKey(vx))
            .map(vertices::get)
    ...
    Set<V> otherVertices =
        graph
            .vertexSet()
            .stream()
            .filter(v -> !oddVertices.contains(v))
            .collect(Collectors.toSet());
    ...
    return matching.getEdges().size()
        == (graph.vertexSet().size() + oddVertices.size() - nrOddCardinalityComponents) / 2.0;
  }
