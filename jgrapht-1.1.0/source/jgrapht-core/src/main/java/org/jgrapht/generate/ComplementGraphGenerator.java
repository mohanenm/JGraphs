...
/**
 * Generator which produces the <a
 * href="http://mathworld.wolfram.com/GraphComplement.html">complement graph</a> of a given input
 ...
 *
 * <p>More formally, let $G = (V, E)$ be a graph and let $K$ consist of all 2-element subsets of
 * $V$. Then $\overline{G} = (V, K \setminus E)$ is the complement of $G$, where $K \setminus E$ is
 * the relative complement of $E$ in $K$. For directed graphs, the complement can be defined in the
 * same way, as a directed graph on the same vertex set, using the set of all 2-element ordered
 * pairs of $V$ in place of the set $K$ in the formula above.
 *
 * <p>The complement is not defined for multigraphs. If a multigraph is provided as input to this
...
public class ComplementGraphGenerator<V, E> implements GraphGenerator<V, E, V> {
  ...
  /**
   * Complement Graph Generator. If the target graph allows self-loops the complement of $G$ may be
   * defined by adding a self-loop to every vertex that does not have one in $G$. This behavior can
   * be controlled using the boolean <code>generateSelfLoops</code>.
   *
   * @param graph input graph
    ...
    if (graph.getType().isDirected()) {
      for (V u : graph.vertexSet())
        for (V v : graph.vertexSet())
          if (u == v) continue;
          else if (!graph.containsEdge(u, v)) target.addEdge(u, v);
      ...
      for (int i = 0; i < vertices.size() - 1; i++) {
        for (int j = i + 1; j < vertices.size(); j++) {
          V u = vertices.get(i);
          V v = vertices.get(j);
          if (!graph.containsEdge(u, v)) target.addEdge(u, v);
        }
      }
    }
    if (generateSelfLoops && target.getType().isAllowingSelfLoops()) {
      for (V v : graph.vertexSet()) {
        if (!graph.containsEdge(v, v)) target.addEdge(v, v);
      }
    }
  }
}
