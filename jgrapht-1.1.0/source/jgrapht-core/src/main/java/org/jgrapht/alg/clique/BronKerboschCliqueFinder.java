 ...
 *
 * <p>Implementation of the Bron-Kerbosch clique enumeration algorithm as described in:
 *
 * <ul>
...
public class BronKerboschCliqueFinder<V, E> extends BaseBronKerboschCliqueFinder<V, E> {
  ...
  protected void lazyRun() {
    if (allMaximalCliques == null) {
      ...
      findCliques(
          new ArrayList<>(), new ArrayList<>(graph.vertexSet()), new ArrayList<>(), nanosTimeLimit);
    }
  }

  private void findCliques(
      List<V> potentialClique,
      List<V> candidates,
      List<V> alreadyFound,
