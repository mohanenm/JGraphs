 ...
 *
 * <p>Computes the closeness centrality of each vertex of a graph. The closeness of a vertex $x$ is
 ...
 * </ul>
 *
 * <p>This implementation computes by default the closeness centrality using outgoing paths and
 * normalizes the scores. This behavior can be adjusted by the constructor arguments.
 *
 * <p>When the graph is disconnected, the closeness centrality score equals $0$ for all vertices. In
 ...
 * (m +n \log n))$ or $O(n^3)$ respectively, where $n$ is the number of vertices and $m$ the number
