 ...
 * true minimal vertex cover is an NP-Complete problem. For more on the vertex cover problem, see <a
 * href="http://mathworld.wolfram.com/VertexCover.html">
 * http://mathworld.wolfram.com/VertexCover.html</a>
 *
 * <p>Note: this class supports pseudo-graphs Runtime: O(|E|*log|V|) This class produces often, but
 * not always, better solutions than the 2-approximation algorithms. Nevertheless, there are
 * instances where the solution is significantly worse. In those cases, consider using {@link
...
public class GreedyVCImpl<V, E> implements MinimumWeightedVertexCoverAlgorithm<V, E> {
   ...
   * algorithm picks the vertex v with the smallest ratio {@code weight(v)/degree(v)} and adds it to
   * the cover. Next vertex v and all edges incident to it are removed. The process repeats until
   * all vertices are covered. Runtime: O(|E|*log|V|)
   *
  ...
  public VertexCover<V> getVertexCover(Graph<V, E> graph, Map<V, Double> vertexWeightMap) {
    ...
    graph
        ...
        .forEach(
            v ->
                vertexEncapsulationMap.put(
                    v, new RatioVertex<>(vertexCounter++, v, vertexWeightMap.get(v))));
    ...
    for (E e : graph.edgeSet()) {
      ...
      assert (ux.neighbors.get(vx) == vx.neighbors.get(ux))
    ...
    assert (workingGraph.size() == vertexEncapsulationMap.size())
        : "vertices in vertexEncapsulationMap: "
    ...
    while (!workingGraph.isEmpty()) { // Continue until all edges are covered
      ...
      assert (workingGraph.parallelStream().allMatch(ux -> vx.getRatio() <= ux.getRatio()))
          : "vx does not have the smallest ratio among all elements. VX: "
              + vx
              + " WorkingGraph: "
              + workingGraph;
      ...
      for (RatioVertex<V> nx : vx.neighbors.keySet()) {
        ...
        if (nx.getDegree() > 0) workingGraph.add(nx);
      }
