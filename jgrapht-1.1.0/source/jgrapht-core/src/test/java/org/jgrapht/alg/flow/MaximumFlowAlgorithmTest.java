...
public abstract class MaximumFlowAlgorithmTest extends MaximumFlowMinimumCutAlgorithmTestBase {
  ...
  static void verifyDirected(
    ...
    for (DefaultWeightedEdge e : flow.keySet()) {
      ...
      assertTrue(flow.get(e) <= (network.getEdgeWeight(e) + EdmondsKarpMFImpl.DEFAULT_EPSILON));
    }
  ...
  static void verifyUndirected(
    ...
    // Verify that every edge is contained in the flow map
    for (DefaultWeightedEdge e : graph.edgeSet()) assertTrue(flow.containsKey(e));
  ...
  public void testDirectedN1() {
    runTestDirected(getDirectedN1(), new int[] {1}, new int[] {4057218}, new double[] {0.0});
  }
  ...
  public void testDirectedN4() {
    runTestDirected(getDirectedN4(), new int[] {1}, new int[] {4}, new double[] {2000000000.0});
  }
