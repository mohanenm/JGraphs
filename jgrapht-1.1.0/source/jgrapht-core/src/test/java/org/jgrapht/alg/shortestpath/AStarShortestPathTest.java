...
public class AStarShortestPathTest extends TestCase {
  ...
  private void readLabyrinth(String[] labyrinth) {
    ...
    for (int i = 0; i < labyrinth.length; i++) {
      for (int j = 0; j < labyrinth[0].length(); j++) {
        if (labyrinth[i].charAt(j) == '#' || labyrinth[i].charAt(j) == ' ') continue;
        nodes[i][j] = new Node(i, j / 2);
        graph.addVertex(nodes[i][j]);
        if (labyrinth[i].charAt(j) == 'S') sourceNode = nodes[i][j];
        else if (labyrinth[i].charAt(j) == 'T') targetNode = nodes[i][j];
      }
    }
    // Create the edges
    // a. Horizontal edges
    for (int i = 0; i < labyrinth.length; i++) {
      for (int j = 0; j < labyrinth[0].length() - 2; j++) {
        if (nodes[i][j] == null || nodes[i][j + 2] == null) continue;
    ...
    for (int i = 0; i < labyrinth.length - 1; i++) {
      for (int j = 0; j < labyrinth[0].length(); j++) {
        if (nodes[i][j] == null || nodes[i + 1][j] == null) continue;
  ...
  public void testLabyrinth1() {
    ...
    GraphPath<Node, DefaultWeightedEdge> path = aStarShortestPath.getPath(sourceNode, targetNode);
    assertNotNull(path);
    ...
    GraphPath<Node, DefaultWeightedEdge> path2 = aStarShortestPath2.getPath(sourceNode, targetNode);
    assertNotNull(path2);
   ...
   * Here we create a simple multigraph A-B-C with multiple edges between (A,B) and (B,C) and query
   * the shortest path, which is simply the cheapest edge between (A,B) plus the cheapest edge
   * between (B,C). The admissible heuristic in this test is not important.
   */
