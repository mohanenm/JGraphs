...
public abstract class BaseColoringTest {
  ...
  protected void testRandomGraphColoring(Random rng) {
    ...
    for (int i = 0; i < tests; i++) {
      ...
      for (Function<Graph<Integer, DefaultEdge>, VertexColoringAlgorithm<Integer>> algProvider :
          algs) {
  ...
  protected final Graph<Integer, DefaultEdge> createGraph1() {
    Graph<Integer, DefaultEdge> g = new SimpleGraph<>(DefaultEdge.class);
  ...
  protected final Graph<Integer, DefaultEdge> createMyciel3Graph() {
  ...
  public void testDSaturNonOptimal() {
    ...
    assertColoring(g, getAlgorithm(g).getColoring(), getExpectedResultOnDSaturNonOptimalGraph());
  }
