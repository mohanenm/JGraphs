...
public class KShortestPathCostTest extends TestCase {
  ...
  public void testKShortestPathCompleteGraph4() {
    ...
    KShortestPaths<String, DefaultWeightedEdge> pathFinder = new KShortestPaths<>(graph, nbPaths);
    ...
    for (String sourceVertex : graph.vertexSet()) {
      for (String targetVertex : graph.vertexSet()) {
        if (targetVertex != sourceVertex) {
          ...
          List<GraphPath<String, E>> pathElements = pathFinder.getPaths(sourceVertex, targetVertex);
   ...
   *
   * @param prevPathElementsResults results obtained with a max-size argument equal to <code>k
   *     </code>
   * @param pathElementsResults results obtained with a max-size argument equal to <code>k+1</code>
   */
