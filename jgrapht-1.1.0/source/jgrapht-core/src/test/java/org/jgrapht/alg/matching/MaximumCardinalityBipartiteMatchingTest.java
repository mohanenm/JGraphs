...
public abstract class MaximumCardinalityBipartiteMatchingTest extends TestCase {
  ...
  private <V, E> void verifyMatching(
    ...
    for (E e : m.getEdges()) {
      ...
      if (matched.contains(source)) fail("vertex is incident to multiple matches in the matching");
      matched.add(source);
      if (matched.contains(target)) fail("vertex is incident to multiple matches in the matching");
      matched.add(target);
