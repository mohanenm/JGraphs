  ...
  public void validateAlgorithm(SimpleWeightedGraph<Integer, DefaultWeightedEdge> network) {
    ...
    double cheapestEdge =
        gomoryHuTree
            .edgeSet()
            .stream()
            .mapToDouble(gomoryHuTree::getEdgeWeight)
            .min()
            .getAsDouble();
    ...
    double cutWeight =
        network
            ...
            .filter(
                e ->
                    partition.contains(network.getEdgeSource(e))
                        ^ partition.contains(network.getEdgeTarget(e)))
            .mapToDouble(network::getEdgeWeight)
            .sum();
      ...
      for (Integer j : network.vertexSet()) {
        ...
        cutWeight =
            network
                ...
                .filter(
                    e ->
                        sourcePartition.contains(network.getEdgeSource(e))
        ...
        DefaultWeightedEdge cheapestEdgeInPath =
            pathEdges
                .stream()
                .min(Comparator.comparing(gomoryHuTreeCopy::getEdgeWeight))
