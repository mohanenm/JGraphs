...
public class JGraphModelAdapter<V, E> extends DefaultGraphModel {
   ...
   * </pre>
   *
   * <p>Step 6 is not always required but we do it anyway as a safeguard against the rare case where
   * the edge to be added is already contained in the graph and thus NO event will be fired. If 6 is
   * not done, a junk edge will remain in the jCellsBeingAdded set.
   *
   * <p>The other sets are used in a similar manner to the above. Apparently, All that complication
  ...
  public JGraphModelAdapter(Graph<V, E> jGraphTGraph) {
    this(jGraphTGraph, createDefaultVertexAttributes(), createDefaultEdgeAttributes(jGraphTGraph));
  }
   ...
   * @return the JGraph edge cell that corresponds to the specified JGraphT edge, or <code>null
   ...
   * @return the JGraph vertex cell that corresponds to the specified JGraphT vertex, or <code>null
   *     </code> if no corresponding cell found.
   */
  ...
  /**
   * Removes the specified JGraphT edge from being reflected by this graph model. To be called only
   * for edges that have already been removed from the JGraphT graph.
   *
  ...
  /**
   * Tests if the specified JGraph edge is 'dangling', that is having at least one endpoint which is
   ...
   * originate there.
   *
   * <p>If this change contains changes that would make this an illegal JGraphT graph, like adding
   * an edge that is incident with only one vertex, the illegal parts of the change are undone.
   */
  private class JGraphListener implements GraphModelListener, Serializable {
    ...
    /**
     * Filters a list of edges out of an array of JGraph GraphCell objects. Other objects are thrown
     * away.
     *
