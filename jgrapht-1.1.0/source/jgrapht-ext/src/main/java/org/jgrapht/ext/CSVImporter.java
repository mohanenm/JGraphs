 ...
 *
 * <p>The importer supports various different formats which can be adjusted using the {@link
 * #setFormat(CSVFormat) setFormat} method. The supported formats are the same CSV formats used by
 * <a href="https://gephi.org/users/supported-graph-formats/csv-format">Gephi </a>. For some of the
 * formats, the behavior of the importer can be adjusted using the {@link
 * #setParameter(org.jgrapht.ext.CSVFormat.Parameter, boolean) setParameter} method. See {@link
 * CSVFormat} for a description of the formats.
 *
 * <p>The importer respects <a href="http://www.ietf.org/rfc/rfc4180.txt">rfc4180</a>. The caller
 * can also adjust the separator to something like semicolon or pipe instead of comma. In such a
 * case, all fields are unescaped using the new separator. See <a
 * href="https://en.wikipedia.org/wiki/Delimiter-separated_values">Delimiter- separated values</a>
 * for more information.
 *
 * <p>This importer does not distinguish between {@link CSVFormat#EDGE_LIST} and {@link
 * CSVFormat#ADJACENCY_LIST}. In both cases it assumes the format is {@link
...
public class CSVImporter<V, E> implements GraphImporter<V, E> {
  ...
  public CSVImporter(
    ...
    switch (format) {
      case ADJACENCY_LIST:
        this.delegate =
            new org.jgrapht.io.CSVImporter<>(
                delegateProvider(vertexProvider),
                delegateProvider(edgeProvider),
                org.jgrapht.io.CSVFormat.ADJACENCY_LIST,
                delimiter);
        break;
      case EDGE_LIST:
        this.delegate =
      ...
      case MATRIX:
        this.delegate =
            new org.jgrapht.io.CSVImporter<>(
                delegateProvider(vertexProvider),
                delegateProvider(edgeProvider),
                org.jgrapht.io.CSVFormat.MATRIX,
                delimiter);
        break;
    }
  }
  ...
  public boolean isParameter(CSVFormat.Parameter p) {
    switch (p) {
      case MATRIX_FORMAT_EDGE_WEIGHTS:
        return delegate.isParameter(org.jgrapht.io.CSVFormat.Parameter.MATRIX_FORMAT_EDGE_WEIGHTS);
      ...
      case MATRIX_FORMAT_ZERO_WHEN_NO_EDGE:
        return delegate.isParameter(
            org.jgrapht.io.CSVFormat.Parameter.MATRIX_FORMAT_ZERO_WHEN_NO_EDGE);
    }
    return false;
  }
  ...
  public void setParameter(CSVFormat.Parameter p, boolean value) {
    switch (p) {
      case MATRIX_FORMAT_EDGE_WEIGHTS:
        delegate.setParameter(org.jgrapht.io.CSVFormat.Parameter.MATRIX_FORMAT_EDGE_WEIGHTS, value);
        break;
      case MATRIX_FORMAT_NODEID:
   ...
   * example if the input contains self-loops then the graph provided must also support self-loops.
