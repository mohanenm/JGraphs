...
public class DOTImporter<V, E> implements GraphImporter<V, E> {
   ...
   *
   * <p>The current implementation reads the whole input as a string and then parses the graph.
   *
  ...
  private int processHeader(
    ...
    if (current == '{') {
      ...
      if (graph instanceof AbstractBaseGraph
          ...
          && headerParts[i].toLowerCase().equals(DOTUtils.DONT_ALLOW_MULTIPLE_EDGES_KEYWORD)) {
        throw new ImportException(
            "graph defines "
                + DOTUtils.DONT_ALLOW_MULTIPLE_EDGES_KEYWORD
                + " but Multigraph given.");
      } else if (headerParts[i].toLowerCase().equals(DOTUtils.DONT_ALLOW_MULTIPLE_EDGES_KEYWORD)) {
        i = i + 1;
      }
  ...
  /**
   * When we start a new section of the graph we don't know what it is going to be. We work in here
   * until we can work out what type of section this is.
   *
   * @param input the input string to read from.
