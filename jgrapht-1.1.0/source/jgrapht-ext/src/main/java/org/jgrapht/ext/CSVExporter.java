 ...
 *
 * <p>The exporter supports three different formats which can be adjusted using the {@link
 ...
 * formats, the behavior of the exporter can be adjusted using the {@link
 ...
 *
 * <p>The default output respects <a href="http://www.ietf.org/rfc/rfc4180.txt">rfc4180</a>. The
 * caller can also adjust the separator to something like semicolon or pipe instead of comma. In
 * such a case, all fields are escaped using the new separator. See <a
 ...
 */
@Deprecated
public class CSVExporter<V, E> implements GraphExporter<V, E> {
  ...
  private void exportAsMatrix(Graph<V, E> g, PrintWriter out) {
    ...
    boolean exportEdgeWeights = parameters.contains(CSVFormat.Parameter.MATRIX_FORMAT_EDGE_WEIGHTS);
